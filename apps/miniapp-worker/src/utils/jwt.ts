...
export function parseJwtPayload(token: string): { exp?: number } | null {
  try {
    ...
    if (!part) {
      console.info('[worker-jwt] parse fail: no payload part, segs=', token.split('.').length);
      return null;
    }
    const json = utf8FromBytes(decodeBase64Url(part));
    const payload = JSON.parse(json) as { exp?: number };
    console.info(
      '[worker-jwt] decode ok, hasExp=',
    ...
    console.info('[worker-jwt] parse fail:', msg);
    return null;
  }
}
...
/** 解不出 exp 时返回 unknown，不能当成已过期，否则真机无 atob 会刚登录就被踢。 */
export function readJwtExpiry(token: string, skewSeconds = 30): JwtExpiry {
  const payload = parseJwtPayload(token);
  if (!payload?.exp) {
    console.info('[worker-jwt] expiry=unknown (decode failed or no exp)');
    return 'unknown';
  }
