      ...
      <view class="duration-row">
        ...
        <view class="stepper-ctrl">
          ...
          <view class="ctrl-val">
            ...
            <text class="val-unit">小时</text>
          </view>
          <view class="ctrl-btn" :class="{ disabled: store.duration >= 8 }" @tap="changeDuration(1)">
            <text class="ctrl-text">+</text>
          </view>
        </view>
      </view>
      ...
      <view class="section-wrap">
        ...
        <view class="order-card">
          <view class="order-row">
            <text class="order-key">服务类型</text>
            <text class="order-val">{{ store.selectedCatalog?.name ?? '-' }}</text>
          </view>
          <view class="order-row">
            <text class="order-key">服务时长</text>
            <text class="order-val">{{ store.duration }} 小时</text>
          </view>
          <view class="order-row">
...
async function loadCatalogs() {
  ...
  try {
    catalogs.value = await fetchCleaningCatalogs();
    // 默认选中第一条
    if (catalogs.value.length > 0 && !store.selectedCatalog) {
      store.selectedCatalog = catalogs.value[0];
    }
    console.info('[booking-cleaning] catalogs loaded, count=', catalogs.value.length);
  } catch (e) {
    console.info('[booking-cleaning] load catalogs failed', e);
...
function changeDuration(delta: number) {
  const next = store.duration + delta;
  if (next >= 1 && next <= 8) {
    store.duration = next;
  }
}
...
const calMonth = ref(today.getMonth() + 1); // 1-12
...
async function loadDefaultAddress() {
  ...
  if (store.selectedAddress) return; // 已有缓存
  try {
    ...
    console.info('[booking-cleaning] load address failed', e);
  }
}

function goAddressSelect() {
  uni.navigateTo({ url: '/pages/address-select/index?from=cleaning' });
}
...
async function submitOrder() {
  ...
  try {
    ...
    const result = await createCleaningOrder({
      ...
      serviceDuration: store.duration,
