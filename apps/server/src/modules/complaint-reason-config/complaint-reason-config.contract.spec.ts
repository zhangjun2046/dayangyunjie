...
describe('ComplaintReasonConfig database static contract', () => {
  ...
  it('schema 使用自增配置主键、唯一标签，投诉侧存 JSON 快照且不建外键', () => {
    ...
    // 多选快照：complaints.reasons 存 [{ configId, label }]
    expect(schema).toMatch(/model Complaint\s*\{[\s\S]*?reasons\s+Json/);

    // 不保留单值列与外键，配置删除不应影响历史投诉
    expect(schema).not.toMatch(/reasonConfigId\s+Int\?/);
    expect(schema).not.toMatch(/reasonLabel\s+String/);
    expect(schema).not.toMatch(/reasonConfig\s+ComplaintReasonConfig\?\s+@relation/);
    // 配置模型不再持有投诉反向关系（Resident 等其它模型的 complaints 关系不受影响）
    expect(schema).toMatch(
      /model ComplaintReasonConfig\s*\{(?:(?!\}|complaints\s+Complaint\[\])[\s\S])*\}/,
    );
  });
