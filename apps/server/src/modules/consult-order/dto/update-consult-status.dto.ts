...
/** 合法的目标状态枚举（排除 PENDING，因为创建时已默认为 PENDING） */
const ALLOWED_TARGET_STATUSES = [ConsultStatus.FOLLOWING_UP, ConsultStatus.COMPLETED] as const;

export class UpdateConsultStatusDto {
  @ApiProperty({
    description: '目标状态（FOLLOWING_UP=跟进中，COMPLETED=已完成）',
    enum: ALLOWED_TARGET_STATUSES,
    example: ConsultStatus.FOLLOWING_UP,
  })
  ...
  @ApiPropertyOptional({ description: '备注（跟进说明等）', example: '已电话联系客户，安排阿姨周五上门' })
  @IsOptional()
  @IsString()
  @MaxLength(512)
  remark?: string;
