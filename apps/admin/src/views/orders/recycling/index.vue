  ...
  <div class="recycling-page">
    ...
    <el-card shadow="never" class="table-card">
      ...
      >
        ...
        <el-table-column label="服务类型" prop="serviceItem" width="110" />
        ...
        <el-table-column label="预估重量" width="90" align="center">
          <template #default="{ row }">
            {{ row.estimatedWeight != null ? row.estimatedWeight + ' kg' : '—' }}
          </template>
        </el-table-column>
        <el-table-column label="服务地址" min-width="150" show-overflow-tooltip>
          <template #default="{ row }">
    ...
    >
      ...
      >
        <el-row :gutter="16">
          ...
          <el-col :span="12">
            <el-form-item label="预估重量" prop="estimatedWeight" required>
              <div class="duration-stepper">
                <el-button
                  :disabled="createForm.estimatedWeight <= 5"
                  circle
                  size="small"
                  @click="createForm.estimatedWeight = Math.max(5, createForm.estimatedWeight - 5)"
                >-</el-button>
                <span class="duration-value">{{ createForm.estimatedWeight }} kg</span>
                <el-button
                  :disabled="createForm.estimatedWeight >= 200"
                  circle
                  size="small"
                  @click="createForm.estimatedWeight = Math.min(200, createForm.estimatedWeight + 5)"
                >+</el-button>
              </div>
            </el-form-item>
          </el-col>
...
import {
  fetchRecyclingOrders,
  fetchRecyclingOrderDetail,
  createRecyclingOrder,
  assignRecyclingOrder,
  fetchOrderReview,
  type AddressSnapshot,
  type RecyclingOrderItem,
  type RecyclingOrderDetail,
  type ReviewDto,
  type RecyclingOrderStatus,
  type CreateRecyclingOrderDto,
} from '@/api/recycling';
...
type TabValue = RecyclingOrderStatus | '';
...
const timelineNodes: { status: RecyclingOrderStatus; label: string }[] = [
...
const STATUS_ORDER: RecyclingOrderStatus[] = [
  'PENDING_ASSIGN',
  'ASSIGNED',
  'ACCEPTED',
  'IN_SERVICE',
...
const orders = ref<RecyclingOrderItem[]>([]);
const total = ref(0);
const keyword = ref('');
...
/** 前端关键词多条件过滤（补充后端 keyword 之外的本地兜底） */
...
const loadOrders = async () => {
  ...
  try {
    ...
    if (dateRange.value) {
      params.appointDateFrom = dateRange.value[0];
      params.appointDateTo = dateRange.value[1];
    }

    const res = await fetchRecyclingOrders(params as Parameters<typeof fetchRecyclingOrders>[0]);
    orders.value = res.data.data?.items ?? [];
    total.value = res.data.data?.total ?? 0;
    console.info('[RecyclingOrders] loaded', orders.value.length, 'orders, total=', total.value, 'params=', params);
  } catch (e) {
    console.error('[RecyclingOrders] load failed', e);
  } finally {
...
const statusLabelMap: Record<RecyclingOrderStatus, string> = {
  PENDING_ASSIGN: '待派单',
  ASSIGNED: '已派单',
  ACCEPTED: '已接单',
  IN_SERVICE: '服务中',
...
const statusLabel = (status: RecyclingOrderStatus) => statusLabelMap[status] ?? status;

const statusTagType = (
  status: RecyclingOrderStatus,
): '' | 'success' | 'warning' | 'danger' | 'info' => {
  const map: Record<RecyclingOrderStatus, '' | 'success' | 'warning' | 'danger' | 'info'> = {
...
const formatAddress = (snapshot: AddressSnapshot | string | null | undefined): string => {
  if (!snapshot) return '—';
  const obj: AddressSnapshot =
    typeof snapshot === 'string'
      ? (() => { try { return JSON.parse(snapshot); } catch { return {}; } })()
      : snapshot;
  return (
    [obj.province, obj.city, obj.district, obj.detail, obj.buildingInfo]
      .filter(Boolean)
      .join('') || '—'
  );
};
...
const isNodeDone = (currentStatus: RecyclingOrderStatus, nodeStatus: RecyclingOrderStatus) => {
...
const isNodeActive = (currentStatus: RecyclingOrderStatus, nodeStatus: RecyclingOrderStatus) => {
  return currentStatus === nodeStatus;
};

const getNodeType = (
  currentStatus: RecyclingOrderStatus,
  nodeStatus: RecyclingOrderStatus,
...
const photoGroups = computed(() => {
  ...
  return [
    { type: 'BEFORE', label: '服务前照片', photos: photos.filter((p) => p.photoType === 'BEFORE') },
    { type: 'AFTER', label: '服务后照片', photos: photos.filter((p) => p.photoType === 'AFTER') },
...
const idleWorkers = computed(() => filterAssignableWorkers(allWorkers.value, 'RECYCLING'));
...
const openAssign = async (row: RecyclingOrderItem) => {
  ...
  if (allWorkers.value.length === 0) {
    ...
    try {
      ...
      console.info('[RecyclingOrders] loaded workers', allWorkers.value.length);
    } catch (e) {
      console.error('[RecyclingOrders] load workers failed', e);
    } finally {
...
const submitAssign = async () => {
  ...
  try {
    await assignRecyclingOrder(assignDialog.orderId, assignDialog.selectedWorkerId);
    ElMessage.success('派单成功');
    assignDialog.visible = false;
    loadOrders();
  } catch (e) {
    console.error('[RecyclingOrders] assign failed', e);
  } finally {
...
const createForm = reactive<CreateRecyclingOrderDto & { isProxyOrder: boolean }>({
  serviceItem: '',
  estimatedWeight: 5,
  appointDate: '',
  appointTimeSlot: '',
...
const openCreateDialog = async () => {
  ...
  if (catalogItems.value.length === 0) {
    try {
      ...
      console.error('[RecyclingOrders] load catalogs failed', e);
    }
  }
};
...
const submitCreate = async () => {
  ...
  try {
    const payload: CreateRecyclingOrderDto = {
      serviceItem: createForm.serviceItem,
      estimatedWeight: createForm.estimatedWeight,
    ...
    await createRecyclingOrder(payload);
    ElMessage.success('订单创建成功');
    createDialog.visible = false;
    loadOrders();
  } catch (e) {
    console.error('[RecyclingOrders] create failed', e);
  } finally {
...
const detailDrawer = reactive<{
  visible: boolean;
  loading: boolean;
  order: RecyclingOrderDetail | null;
...
const openDetail = async (row: RecyclingOrderItem) => {
  ...
  try {
    const res = await fetchRecyclingOrderDetail(row.id);
    detailDrawer.order = res.data.data;
    console.info('[RecyclingOrders] detail loaded for', row.orderNo);
    ...
    if (detailDrawer.order?.status === 'REVIEWED') {
      try {
        ...
        console.warn('[RecyclingOrders] review load failed', e);
      }
    }
  } catch (e) {
    console.error('[RecyclingOrders] detail load failed', e);
...
onMounted(() => {
  // P5.12 首页待办卡片跳转预置状态筛选（如 /orders/recycling?status=PENDING_ASSIGN）
