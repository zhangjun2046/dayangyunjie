...
/** 投诉原因已改为动态配置，直接展示落库时的文案快照 */
function formatComplaintReasons(reasons: ComplaintReasonSnapshot[] | undefined): string {
  if (!reasons?.length) return '—';
  return reasons.map((r) => r.label).filter(Boolean).join('、');
}
</script>
