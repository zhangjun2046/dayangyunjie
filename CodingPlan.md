# 大洋云洁 (dayangyunjie) — AI 自动开发计划

> **文档用途**: 本人是非技术人员，使用 WorkBuddy 平台进行 AI 辅助自动开发。
> 本文档是开发的唯一操作手册，按开发单元逐个指导每个环节该做什么、怎么做、怎么验收。

---

## 目录

- [一、项目背景](#一项目背景)
- [二、前置准备清单](#二前置准备清单)
- [三、通用执行流程（每个单元都适用）](#三通用执行流程每个单元都适用)
- [四、模型切换策略](#四模型切换策略)
- [五、上下文继承机制](#五上下文继承机制)
- [六、权限说明总表](#六权限说明总表)
- [七、分阶段开发单元详解](#七分阶段开发单元详解)
  - [P1 基础设施（16h）](#p1-基础设施16h)
  - [P2 后端核心 API（40h）](#p2-后端核心-api40h)
  - [P3 居民端小程序（28h）](#p3-居民端小程序28h)
  - [P4 员工端小程序（20h）](#p4-员工端小程序20h)
  - [P5 管理后台（24h）](#p5-管理后台24h)
  - [P6 集成与部署（12h）](#p6-集成与部署12h)
- [八、工时汇总与排期建议](#八工时汇总与排期建议)
- [九、风险与应急处理](#九风险与应急处理)

---

## 一、项目背景

### 我是谁

我是项目的发起人，**不懂编程和技术细节**。我的角色是：

- **产品经理** — 定义需求（已写入 `requirement/requirement.md`）
- **验收员** — 每个 AI 开发单元完成后检查结果是否可用
- **决策者** — 在关键节点做选择（如确认需求细节、批准方案）

### 什么是 WorkBuddy

WorkBuddy 是一个 AI 编程助手平台。它像一个"会写代码的技术助手"，我通过自然语言告诉它要做什么，它就会：

- 创建和修改代码文件
- 运行终端命令（安装依赖、编译项目等）
- 回答技术问题并给出建议

### 开发模式

```
┌─────────────────────────────────────────────────────┐
│                    开发模式                          │
│                                                     │
│  我（非技术人员）                                     │
│    │                                                │
│    │ 用中文说："开始 P2.5a，实现保洁订单 CRUD"         │
│    ▼                                                │
│  WorkBuddy（AI 助手）                                 │
│    │                                                │
│    ├─ 读取需求文档 / 技术文档 / 数据库设计             │
│    ├─ 理解当前项目已有代码                             │
│    ├─ 自动生成完整代码文件                            │
│    ├─ 运行命令验证编译通过                            │
│    └─ 告诉我完成了，请我检查                           │
│    │                                                │
│    ▼                                                │
│  我（审查 + 反馈）                                    │
│    │                                                │
│    └─ 看生成的文件 → 有问题就反馈 → AI 修正 → 再检查   │
└─────────────────────────────────────────────────────┘
```

### 关键认知

| 事实 | 说明 |
|------|------|
| **我不需要写代码** | 全部由 AI 生成 |
| **我不需要懂技术术语** | 用中文描述需求即可 |
| **但我必须参与** | 每个单元都需要我审查结果、反馈问题、手动测试 |
| **AI 不是全自动** | 它像一个非常能干的实习生，需要我来指导和验收 |

### 已有文档

| 文档 | 路径 | 内容 |
|------|------|------|
| 需求文档 | `D:\coding\dayangyunjie\requirement\requirement.md` | 功能需求、业务流程、原型图描述 |
| 技术选型 | `D:\coding\dayangyunjie\codework\tech.md` | 技术栈、架构约定、编码规范 |
| 数据库设计 | `D:\coding\dayangyunjie\codework\Schema.md` | 13 个数据表、8 个枚举、字段定义 |
| **本开发计划** | `D:\coding\dayangyunjie\codework\CodingPlan.md` | **本文档 — 你正在看的操作指引** |

---

## 二、前置准备清单

在开始第一个开发单元之前，需要完成以下准备工作。按优先级排列：

### 🔴 必须在 Day 1 完成（否则 P1 无法启动）

| # | 准备项 | 怎么做 | 验证方法 |
|---|--------|--------|----------|
| 1 | **切换到 Craft 模式** | 在 WorkBuddy 界面将模式从 Ask 切换到 Craft | 切换后 WorkBuddy 可以创建文件和运行命令 |
| 2 | **Git 初始化 + 配置 GitHub 远程仓库** | 对 AI 说 "在项目目录初始化 git 并配置 GitHub" | 出现 `.git` 文件夹；`git remote -v` 显示远程地址 |
| 3 | **本地 MySQL 可用** | 最简单方式：安装 MySQL 8 或用 Docker 运行 | 终端输入 `mysql -u root -p` 能连上 |

> **GitHub 账号信息**（代码托管平台，用于备份和版本管理）：
>
> - **平台**: GitHub.com
> - **用户名**: `zhangjun2046`
> - **邮箱**: 13810779530@139.com
> - **密码** (已废弃): ~~Zj215319~~
> - **Personal Access Token**: `github_pat_11BQEO...OtkpD`（完整 Token 已存入本地 `.env` 文件，不写入代码库）
> - **远程仓库地址**: `https://github.com/zhangjun2046/dayangyunjie.git`
> - **Token 权限**: All repositories / Administration
>
> ⚠️ **安全提示**: Token 和本文档包含敏感凭证信息，请勿公开分享或上传到公共场合。

### 🟡 必须在 P3 开始前完成（约第 7-8 天）

| # | 准备项 | 说明 |
|---|--------|------|
| 4 | **微信开发者工具** | 从微信官网下载安装，用于预览小程序 |
| 5 | **微信小程序 AppID** | 去[微信公众平台](https://mp.weixin.qq.com)注册，免费获取测试号 |

### ⚪ 必须在 P6 开始前完成（最后 2-3 天）

| # | 准备项 | 说明 |
|---|--------|------|
| 6 | **腾讯云服务器 CVM** | 用于部署上线 |
| 7 | **腾讯云 COS 存储桶** | 用于存储照片 |
| 8 | **域名 + SSL 证书** | 用于 HTTPS 访问 |

> **注意**: 第 4-8 项不需要现在准备。先把第 1-3 项搞定就可以开工了。

---

## 三、通用执行流程（每个单元都适用）

无论哪个开发单元，执行流程都是一样的。把这个流程刻在脑子里：

```
┌──────────────────────────────────────────────────────────┐
│                                                           │
│  第一步: 启动                                            │
│  ─────────────────                                      │
│ 我对 WorkBuddy 说：                                       │
│  "开始 [单元编号]，[一句话描述要做什么]"                     │
│                                                           │
│  例如:                                                    │
│  "开始 P1.2，把 Schema.md 转译为 Prisma Schema 文件"       │
│  "开始 P2.5a，实现 CleaningOrder CRUD 模块"               │
│  "开始 P3.3，实现居民端保洁预约三步向导页面"               │
│                                                           │
├──────────────────────────────────────────────────────────┤
│                                                           │
│  第二步: AI 执行                                         │
│  ────────────────────                                   │
│  WorkBuddy 自动做以下事情：                                │
│  ✓ 读取相关文档（需求 / 技术 / 数据库设计）                │
│  ✓ 读取项目中已有的代码                                   │
│  ✓ 生成/修改代码文件                                      │
│  ✓ 运行命令验证（如 npm run build）                       │
│  ✓ 告诉我完成了                                          │
│                                                           │
│  ★ 这一步我不需要做任何事情，等待即可                       │
│  ★ 根据单元复杂度，可能等待 2-10 分钟                      │
│                                                           │
├──────────────────────────────────────────────────────────┤
│                                                           │
│  第三步: 我来审查 ⚠️ 重要！                               │
│  ─────────────────────                                  │
│  AI 完成后，告诉我它做了什么。然后我需要：                  │
│                                                           │
│  ① 看一眼 AI 列出的"生成了哪些文件"                        │
│     → 数量对不对？文件名合理吗？                           │
│                                                           │
│  ② 让 AI 展示关键文件的内容                               │
│     → 不需要看懂每一行代码                                │
│     → 只需要看：有没有明显的遗漏（比如某个功能没实现）      │
│                                                           │
│  ③ 如果发现问题，直接用中文告诉 AI                         │
│     → "ServiceCatalog 缺了 sortOrder 字段"               │
│     → "登录页没有隐私协议弹窗"                             │
│     → AI 会立刻修复                                       │
│                                                           │
│  ④ 如果没问题或修复后 OK 了，说 "继续下一个单元"            │
│                                                           │
├──────────────────────────────────────────────────────────┤
│                                                           │
│  第四步: 测试验收 ⚠️ 必做！                              │
│  ─────────────────────                                  │
│  每个单元都有明确的"测试标准"（见下方各单元详细说明）。       │
│  我需要按照标准实际操作一遍，确认能用。                      │
│                                                           │
│  测试方式因单元类型而异（见下方）：                          │
│  - 后端 API 单元 → 用 Swagger 页面点一下接口              │
│  - 小程序页面 → 用微信开发者工具走一遍流程                 │
│  - 管理后台 → 用浏览器打开页面操作一遍                     │
│  - 配置/脚手架 → 看能不能正常启动/编译                      │
│                                                           │
│  ★ 测试通过了才能进入下一个开发单元                          │
│  ★ 测试不通过 → 告诉 AI 问题现象 → AI 修复 → 重新测试      │
│                                                           │
└──────────────────────────────────────────────────────────┘
```

### 我在每个环节的具体动作总结

| 环节 | 我做什么 | 花多少时间 |
|------|---------|-----------|
| 启动 | 对 AI 说一句话启动单元 | 10 秒 |
| AI 执行 | **什么都不做，等待** | 2-10 分钟 |
| 审查 | 看文件列表 + 浏览关键内容 + 反馈问题 | 10-30 分钟 |
| 测试验收 | 按测试标准手动操作验证 | 15-45 分钟 |

**标注 2h 的开发单元，实际我需要投入约 30-75 分钟（含审查+测试）。**

---

## 四、模型切换策略

WorkBuddy 使用两种 AI 模型来完成不同类型的开发工作：

| 模型 | 代号 | 特点 | 适用场景 |
|------|------|------|----------|
| Auto | 默认模型 | 通用能力强，适合标准操作 | 脚手架搭建、CRUD 模板、配置文件 |
| DeepSeek V4 Pro | DS V4 Pro | 专注编程，代码质量更高 | 复杂业务逻辑、状态机、前端页面 |

### 分配规则

```
┌────────────────────────────────────────────────────────┐
│  阶段          工时     模型        切换时机             │
│────────────────────────────────────────────────────────│
│  P1 基础设施    16h      Auto        不需切换            │
│  P2.1~2.4      12h      Auto        不需切换            │
│  P2.5a~2.6b    12h      DS V4 Pro   ← 进入 P2.5a 时切换 │
│  P2.7~2.11     16h      DS V4 Pro   继续使用            │
│  P3 全部       28h      DS V4 Pro   继续使用            │
│  P4 全部       20h      DS V4 Pro   继续使用            │
│  P5.1~5.4      15h      Auto/DS     ← 可切回 Auto       │
│  P5.5~5.8       9h      Auto        继续使用            │
│  P6 全部       12h      Auto        继续使用            │
└────────────────────────────────────────────────────────┘
```

### 如何切换模型

在 WorkBuddy 界面中找到模型设置选项，选择对应的模型即可。具体操作方式以界面为准。

> **重要提示**: 第一次切换到 DS V4 Pro 时，我会（AI）主动提醒你并生成上下文摘要文档，确保新模型能接上之前的工作成果。

---

## 五、上下文继承机制

### 为什么需要这个

当从一个模型切换到另一个模型时（例如从 Auto 切换到 DS V4 Pro），新模型看不到之前的对话记录。就像换了一个新的技术助手，它不知道之前做了什么。

### 解决办法

在关键节点，AI 会**自动生成一份"交接文档"**，包含：

- 已完成的开发单元列表
- 已创建的文件及其作用
- 重要的类名/方法名/API 接口地址
- 下一个单元需要知道的关键信息

### 自动生成交接文档的节点

| 节点 | 生成的文档 | 内容 |
|------|-----------|------|
| P2.4 完成后 → 进入 P2.5a 前 | `OrderModule-API-Contract.md` | 用户/认证模块的全部 API 接口列表 |
| P2.11 完成后 → 进入 P3 前 | `Backend-API-Summary.md` | 后端全部接口路径/参数/返回值，供小程序对接 |
| P3 完成后 → 进入 P4 前 | `MiniApp-Architecture.md` | 居民端的页面结构、store 设计、组件复用说明 |

**这些文档不需要我自己编写，AI 会在到达这些节点时自动提示并生成。**

---

## 六、权限说明总表

### AI（WorkBuddy）已有的能力

| 能力 | 状态 | 说明 |
|------|------|------|
| 读写项目文件 | ✅ 已有 | 可以创建/修改/删除代码文件 |
| 运行终端命令 | ✅ 已有（Craft 模式下） | 可以运行 npm、npx 等命令 |
| Node.js 环境 | ✅ 已有 | Node.js 22.22.2 已安装 |
| Python 环境 | ✅ 已有 | Python 3.13.12 已安装 |

### 需要我提供的资源/账号

| 资源 | 何时需要 | 怎么给 AI | 安全注意事项 |
|------|---------|----------|-------------|
| **GitHub 账号**（代码托管） | P1 起 | 已记录用户名 `zhangjun2046` + Token（见上方账号信息块） | AI 会在 P1 配置 git remote 和推送 |
| MySQL 连接信息 | P1.2 起 | 告诉 AI 地址、端口、用户名、密码 | 开发期用简单密码即可 |
| 微信小程序 AppID | P3/P4 起 | 告诉 AI AppID 和 AppSecret | Secret 写入 .env，不提交到 git |
| 腾讯云 COS 密钥 | P2.9 起 | 告诉 AI SecretId 和 SecretKey | 同上，写入 .env |

### 我的账号速查

| 平台 | 用户名/邮箱 | 用途 |
|------|------------|------|
| **GitHub** | `zhangjun2046` / `13810779530@139.com` | 代码托管、版本备份（P1 起全程使用） |
| Token | `github_pat_11BQEO...OtkpD`（完整 Token 见上方 GitHub 账号信息块） | 用于推送认证（替代密码） |

### AI 做不到的事

| 事项 | 原因 | 替代方案 |
|------|------|----------|
| 登录腾讯云控制台 | AI 没有浏览器 | 我自己登录操作，然后把需要的密钥告诉 AI |
| 登录微信公众平台 | 同上 | 同上 |
| 手机扫码授权 | 需要真人操作 | 我自己在手机上操作 |
| 注册/实名认证账号 | 需要身份证/手机号 | 我自己注册 |

---

## 七、分阶段开发单元详解

### P1 基础设施（16h）

**目标**: 搭建整个项目的基础骨架，让三个端（后端/居民小程序/管理后台）都能跑起来。

---

#### P1.1 NestJS 项目脚手架 + Prisma 初始化（3h）

**干什么**

创建后端项目的基本结构。包括：
- NestJS 项目初始化（类似盖房子先搭框架）
- 安装 Prisma（数据库工具）
- 配置基础的项目结构

**WorkBuddy 干什么**
- 创建 `apps/server/` 目录及全部基础文件
- 配置 `package.json`、`tsconfig.json`、`nest-cli.json`
- 安装所需依赖包
- 初始化 Prisma 配置
- 运行 `npm run build` 验证编译通过

**人工干什么**
- ✅ 等待 AI 完成（约 5-10 分钟）
- ✅ 确认终端显示 "Build succeeded"
- ✅ 对 AI 说 "继续"

**使用模型**: Auto

**需要权限**: 文件读写、终端命令（npm install）、Node.js 环境

**测试标准 — 通过后方可进入 P1.2**:
1. 终端运行 `cd apps/server && npm run build` 显示成功
2. `apps/server/prisma/` 目录存在且包含 `schema.prisma` 文件（此时为空）

---

#### P1.2 Schema.prisma 编写（4h）

**干什么**

把 `Schema.md`（数据库设计文档）翻译成 Prisma 能理解的格式。这是整个系统的数据基石——所有数据都要存进这个数据库里。

包含：
- 13 个数据模型（用户表、订单表、评价表等）
- 8 个枚举（订单状态、支付状态等）
- 表之间的关联关系（订单属于谁、谁下了单等）
- 索引（加速查询）
- 种子数据（测试用的假数据）

**WorkBuddy 干什么**
- 读取 `Schema.md` 全文
- 生成 `prisma/schema.prisma` 文件（约 300 行）
- 生成种子数据文件 `prisma/seed.ts`
- 运行 `npx prisma migrate dev` 在 MySQL 中建表
- 运行 `npx prisma db seed` 写入种子数据

**人工干什么**
- ✅ 等待 AI 完成
- ✅ AI 会列出"生成了 X 个模型、Y 个枚举"，核对数量是否为 13 个模型 + 8 个枚举
- ✅ 如发现遗漏（如"少了 ServiceCatalog 的 sortOrder 字段"），反馈给 AI
- ✅ 确认终端显示 migrate 成功（数据库表已建立）

**使用模型**: Auto

**需要权限**: 文件读写、终端命令（prisma migrate）、MySQL 连接

**测试标准 — 通过后方可进入 P1.3**:
1. `npx prisma migrate dev` 成功执行，无报错
2. `npx prisma studio` 打开后能看到全部 13 张表的空表结构
3. `npx prisma db seed` 成功写入种子数据

---

#### P1.3 共享类型包 packages/shared/（2h）

**干什么**

创建一个"共享词典"。因为后端、居民端、管理后台都会用到相同的数据类型定义（如"订单状态有哪些"、"性别枚举是什么"），把它们集中到一个地方，避免重复定义导致不一致。

**WorkBuddy 干什么**
- 创建 `packages/shared/` 目录
- 定义全部 DTO（数据传输对象）、枚举、通用响应类型
- 配置 TypeScript 编译
- 验证三个 app 都能 import 这个包

**人工干什么**
- ✅ 确认编译通过
- ✅ 无需深入查看具体代码（这是纯机械性的类型定义）

**使用模型**: Auto

**需要权限**: 文件读写、Node.js

**测试标准 — 通过后方可进入 P1.4**:
1. `npm run build` 无报错
2. AI 演示从三个 app 分别 import shared 包中的枚举（如 `OrderStatus`）不报错

---

#### P1.4 uni-app 双端骨架（4h）

**干什么**

创建两个小程序项目的基本框架：
- **居民端**: 居民使用的微信小程序（下单、查订单、评价）
- **员工端**: 清洁员工使用的微信小程序（接单、签到、拍照）

两个项目共用一套 uni-app 代码库，但通过条件编译区分不同功能。

**WorkBuddy 干什么**
- 创建 uni-app 项目结构
- 配置底部导航栏（首页/订单/我的）
- 配置微信小程序编译选项
- 安装 UI 组件库（uni-ui）
- 验证 H5 模式可编译运行

**人工干什么**
- ✅ 确认 H5 模式可以启动（浏览器打开能看到页面）
- ✅ 底部导航栏可以看到几个 tab（即使点击后是空白页也正常）
- ✅ 确认有两个入口：居民端、员工端

**使用模型**: Auto

**需要权限**: 文件读写、Node.js、npm

**测试标准 — 通过后方可进入 P1.5**:
1. H5 开发模式启动成功，浏览器可见底部 tabbar
2. 点击不同 tab 能切换页面（内容可以是空的）

---

#### P1.5 Vue 3 管理后台脚手架（3h）

**干什么**

创建管理后台（物业管理人员使用的网页系统）的基础框架。基于一个叫 vue3-element-admin 的开源模板搭建。

**WorkBuddy 干什么**
- 基于 youlai 模板初始化 Vue 3 项目
- 配置路由系统（页面跳转规则）
- 配置布局框架（侧栏菜单 + 顶部导航 + 内容区）
- 封装 HTTP 请求模块（axios）
- 配置登录页
- 验证 `npm run dev` 可启动

**人工干什么**
- ✅ 浏览器打开管理后台地址
- ✅ 能看到登录页面
- ✅ 能看到侧栏菜单（即使还没连接真实登录功能）

**使用模型**: Auto

**需要权限**: 文件读写、Node.js、npm

**测试标准 — 通过后 P1 阶段完成**:
1. `npm run dev` 启动成功
2. 浏览器访问看到登录页渲染正常
3. 侧栏布局框架可见

**🎉 P1 阶段验收标准**: 后端 `POST /api/auth/login` 能返回 JWT token；前端三个端均可启动。

---

### P2 后端核心 API（40h）

**目标**: 实现后端全部业务逻辑接口。这是整个系统的大脑——小程序和管理后台的所有数据交互都靠这里。

> **注意**: 从 P2.5a 开始切换到 **DeepSeek V4 Pro** 模型。

---

#### P2.1 Auth 模块 — JWT + 微信登录（6h）

**干什么**

实现用户身份认证系统。就像小区的门禁系统——谁来了要验证身份，验过了才放行。

包含：
- 微信登录：用户在小程序里点"授权登录"→ 获取 openid → 系统识别用户身份
- JWT 令牌：登录成功后发一张"通行证"（token），后续每次请求带着这张证
- Token 刷新：通行证过期了可以换新的

**WorkBuddy 干什么**
- 创建 WechatModule（处理微信登录逻辑）
- 实现 JwtStrategy（JWT 验证策略）
- 创建 Guard（门卫，拦截未登录请求）
- 配合 Swagger 提供可测试的接口文档

**人工干什么**
- ✅ 理解概念即可（无需看代码）：微信登录 = 用微信号当身份证；JWT = 临时通行证
- ✅ 让 AI 用 Swagger 演示登录流程

**使用模型**: Auto

**需要权限**: 文件读写、终端、微信 AppID（可先用假值 mock）

**测试标准 — 通过后方可进入 P2.2**:
1. Swagger UI (`http://localhost:3000/api-docs`) 打开可见 Auth 相关接口
2. POST `/api/auth/wechat-login` 传入微信 code 能返回 token（mock 模式下返回假 token）
3. 带 token 访问受保护接口返回数据（不带 token 返回 401 未授权）

---

#### P2.2 用户 CRUD 模块（5h）

**干什么**

实现对三类用户的增删改查操作：
- **Resident（居民）**：小区住户的信息管理
- **Worker（清洁员工）**：服务人员的信息管理
- **Admin（管理员）**：后台管理人员的信息管理

**WorkBuddy 干什么**
- 为三张用户表分别生成 Controller + Service
- 实现 POST（新增）/ GET（查询）/ PUT（修改）/ DELETE（删除）接口
- 密码加密存储（bcrypt，存的是加密后的密码，不是明文）
- 接入 Swagger 文档

**人工干什么**
- ✅ 在 Swagger 页面上逐个点一下四个接口
- ✅ 新增一条测试数据 → 查询能查到 → 修改后再查 → 删除后查不到

**使用模型**: Auto

**需要权限**: 文件读写、终端、MySQL

**测试标准 — 通过后方可进入 P2.3**:
1. 三类用户的 CRUD 四个接口在 Swagger 中均可调用
2. 新增的用户密码经过加密存储（数据库中不是明文密码）

---

#### P2.3 地址管理模块（2h）

**干什么**

居民的收货/服务地址管理。类似外卖 app 里的地址簿功能。

包含：
- 新增地址
- 编辑地址
- 删除地址
- 设置默认地址

**WorkBuddy 干什么**
- 生成 Address CRUD 全套接口
- 实现默认地址逻辑（设一个为默认时自动取消其他的默认标记）
- 绑定到 Resident 用户

**人工干什么**
- ✅ 用 Swagger 新增 3 个地址 → 设其中一个为默认 → 查询确认只有 1 个默认 → 删除 1 个非默认地址

**使用模型**: Auto

**需要权限**: 同上

**测试标准 — 通过后方可进入 P2.4**:
1. 新增/编辑/删除/设默认地址四个接口均可用
2. 设默认后只有一个地址的 isDefault=true

---

#### P2.4 服务目录模块（2h）

**干什么**

维护平台提供的服务项目列表（保洁类型、废品回收类型等），含参考价格。

**WorkBuddy 干什么**
- 生成 ServiceCatalog 查询接口
- 支持按 bizType（保洁/废品/家政）筛选
- 返回服务名称、参考价、图标等信息

**人工干什么**
- ✅ 用 Swagger 查询服务列表，确认能看到保洁/废品/家政分类的数据

**使用模型**: Auto

**需要权限**: 同上

**⚠️ P2.4 完成后，AI 会自动生成 `OrderModule-API-Contract.md` 交接文档。**

**测试标准 — 通过后准备切换模型进入 P2.5a**:
1. 小程序端可通过 API 获取三种服务的列表（含参考价格）
2. 数据来自 P1.2 写入的种子数据

---

#### P2.5a CleaningOrder CRUD + 创建订单（2h）🔄 **此处切换到 DS V4 Pro**

**干什么**

实现保洁订单的基础数据操作。注意这只是"创建和查询订单"，还不涉及状态流转（那是下一个单元的事）。

包含：
- 创建保洁订单（居民提交预约请求）
- 订单列表（支持分页、筛选）
- 订单详情
- 订单编号自动生成（CLN 前缀 + 时间戳）

**WorkBuddy 干什么**
- 读取 `OrderModule-API-Contract.md`（上一节点 AI 生成的交接文档）
- 创建 CleaningOrder Controller + Service
- 实现 create/list/getOne/update 方法
- 订单号生成规则：CLN + yyyyMMdd + 6位序号
- 计算 referenceAmount（预估金额 = 服务时长 × 参考单价）

**人工干什么**
- ✅ 用 Swagger POST 创建一个保洁订单 → 返回订单号以 CLN 开头
- ✅ 用 GET 查到刚创建的订单 → 信息一致

**使用模型**: **DeepSeek V4 Pro** ⬅️ **从这里开始切换**

**需要权限**: 文件读写、终端、MySQL

**测试标准 — 通过后方可进入 P2.5b**:
1. POST 创建订单返回 CLN 前缀订单号
2. GET 列表/详情能正确返回数据
3. referenceAmount 计算正确

---

#### P2.5b CleaningOrder 状态机核心（3h）

**干什么**

这是**整个系统最核心、最复杂的业务逻辑**。定义保洁订单的状态流转规则——订单从"下单"到"最终完成"中间要经历哪些步骤，哪些转换是合法的，哪些是不允许的。

保洁订单的 6 个状态：

```
待派单(PENDING_ASSIGN) → 已派单(ASSIGNED) → 已接受(ACCEPTED)
                                                  ↓
                              服务中(IN_SERVICE) ← 接单后GPS签到
                                                  ↓
                              待评价(PENDING_REVIEW) ← 员工完成服务
                                                  ↓
                              已评价(REVIEWED) ← 居民打分评价
                                                  ↓
                              已取消(CANCELLED) ← 任意非终态均可取消
```

**WorkBuddy 干什么**
- 设计独立的 `OrderStateMachine` 类/方法集
- 实现每种状态的合法转移规则
- 非法转移抛出明确异常（如"不能从已完成状态退回待派单"）
- 编写状态流转测试用例
- 维护 order_status_log 审计日志（每次状态变更都留记录）

**人工干什么**
- ✅ **重点审查**: AI 会输出一张"状态流转规则表"，我需要对照需求文档确认每条规则正确
- ✅ 特别关注：取消操作的规则（哪些状态下允许取消？取消后退款？）
- ✅ 让 AI 跑一遍测试用例，所有状态流转场景都通过

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P2.5c**:
1. 每种合法状态跳转都能成功执行
2. 每种非法状态跳转都被正确拒绝并给出错误提示
3. order_status_log 每次状态变更都有记录
4. AI 提供的测试用例全部通过

---

#### P2.5c 派单 + GPS签到 + 完成操作接口（2h）

**干什么**

实现保洁订单的各种"操作按钮"对应的后端逻辑：

- **派单**：管理员分配员工 → 订单变为"已派单"
- **GPS 签到**：员工到达现场 → 上传位置 → 系统判断距离是否在 200m 内 → 订单变为"服务中"
- **完成服务**：员工上传完工照片 → 订单变为"待评价"
- **取消订单**：任意非终态下的取消操作

**WorkBuddy 干什么**
- 基于上一步的状态机，实现各操作 API endpoint
- GPS 校验：接收经纬度 → Haversine 公式计算距离 → 超距标记
- 操作前后触发状态变更（调用 P2.5b 的状态机）
- 返回操作结果和更新后的订单状态

**人工干什么**
- ✅ 用 Swagger 依次调用：创建订单 → 派单 → 模拟接单 → 模拟 GPS 签到 → 完成服务 → 确认状态依次正确变化
- ✅ 测试超距签到（故意传一个很远的坐标）→ 应被标记为异常

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后 P2.5（保洁订单）全部完成**:
1. e2e 全链路：居民下单 → 后台派单 → 员工接单 → GPS 签到 → 上传照片 → 完成 → 状态全程正确
2. 超距签到被正确标记

---

#### P2.6a RecyclingOrder CRUD + 继承状态机（2h）

**干什么**

废品回收订单的基础 CRUD。废品订单比保洁订单多了两个特殊状态（待验收、已验收），其余逻辑类似。

**WorkBuddy 干什么**
- 复用 P2.5b 的状态机模式
- 创建 RecyclingOrder Controller + Service
- 增加 PENDING_ACCEPTANCE / ACCEPTED 两个状态节点
- 订单号前缀改为 RCY

**人工干什么**
- ✅ 与 P2.5a 类似的 CRUD 测试流程
- ✅ 确认废品订单多了"待验收"相关字段

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P2.6b**:
1. POST 创建订单返回 RCY 前缀订单号
2. 废品订单状态机比保洁订单多出"待验收"和"已验收"节点

---

#### P2.6b 废品特有流程（3h）

**干什么**

废品回收独有的业务逻辑：
- **录入实际重量**：员工上门后称重，录入实际重量（可能与预估不同）
- **核定金额计算**：实际重量 × 单价 = 最终收款金额
- **居民验收**：居民确认重量和金额无误
- **收款确认**：线下收款后员工标记"已收款"

**WorkBuddy 干什么**
- 实际重量录入接口（员工调用）
- 核定金额自动计算
- 居民验收接口（居民调用）
- 收款状态变更（paymentStatus: UNPAID → PAID）
- payment_status_log 日志记录

**人工干什么**
- ✅ e2e 测试：创建废品订单 → 派单 → 接单 → 录入重量(10kg) → 确认核定金额正确 → 模拟居民验收 → 模拟收款 → 全链路通过

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后 P2.6（废品订单）全部完成**:
1. e2e 全链路跑通，额外包含「录入重量 → 居民验收」环节
2. 核定金额 = 实际重量 × 单价，计算正确
3. 验收后 paymentStatus 正确变更

---

#### P2.7 ConsultOrder 咨询单模块（3h）

**干什么**

家政咨询需求的处理。相对简单，只有三个状态：

```
待跟进(FOLLOW_UP) → 跟进中(FOLLOWING) → 已完成(COMPLETED)
```

**WorkBuddy 干什么**
- ConsultOrder CRUD
- 简单的三态流转
- 订单号 CNS 前缀

**人工干什么**
- ✅ 提交咨询单 → 后台变更为跟进中 → 变更已完成，状态流转正确

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P2.8**:

---

#### P2.8 GPS 签到校验（2h）

**干什么**

独立的 GPS 距离计算服务。员工上传坐标后，系统计算与服务地址的距离，判断是否在允许范围内。

**WorkBuddy 干什么**
- Haversine 公式实现（根据两点经纬度计算地球表面距离）
- 距离阈值配置（默认 200 米）
- 超距标记 + 异常原因记录
- 独立为可复用的 Service

**人工干什么**
- ✅ 传入一组经纬度 → 看返回的距离数值是否合理（可以用手机 GPS 坐标粗略判断）
- ✅ 传入一个很远的位置 → 确认被标记为超距

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P2.9**:
1. 传入坐标 → 返回距离（米）
2. 超 200m 标记超距并记录说明

---

#### P2.9 COS 文件上传 + 水印（3h）

**干什么**

图片上传功能。员工拍摄的服务照片上传到腾讯云对象存储（COS），并在图片上自动添加水印（批次号 + 时间戳）。

**WorkBuddy 干什么**
- 集成 cos-nodejs-sdk-v5
- 图片上传接口（接收 multipart/form-data）
- sharp 库添加水印文字（订单号 + 当前时间）
- 私有桶存储 + 临时签名 URL 生成
- 开发期先用本地 `/uploads` 目录模拟，部署前切换 COS

**人工干什么**
- ✅ 用 Swagger 上传一张测试图片 → 能访问到图片 URL → 图片上有水印文字

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 文件读写、终端、COS 密钥（开发期可跳过，用本地存储）

**测试标准 — 通过后方可进入 P2.10**:
1. 上传图片后访问 URL 可见水印（含订单号 + 时间）
2. 开发期本地存储模式也能正常工作

---

#### P2.10 评价与投诉模块（3h）

**干什么**

居民对服务的评价和投诉功能。

**评价**：星级（1-5）+ 标签标签（准时/专业/干净等）+ 文字评语 + 图片
**投诉**：投诉内容 + 凭证图片 → 管理人员处理 → 跟进记录 → 结案

**WorkBuddy 干什么**
- Review CRUD（评价）
- Complaint CRUD（投诉）
- ComplaintFollowUp（投诉跟进记录）
- 评价与订单关联

**人工干什么**
- ✅ 提交一次评价（选星、选标签、写文字）→ 查看评价详情 → 数据一致
- ✅ 提交一次投诉 → 添加处理跟进 → 结案

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P2.11**:
1. 评价提交成功，星/标签/文字/图均可保存
2. 投诉提交 → 处理跟进 → 结案，全流程状态正确

---

#### P2.11 数据看板聚合 API（2h）

**干什么**

为管理后台的数据看板提供统计数据。这些接口不做复杂计算，就是把数据库里的数据聚合成图表能用的格式。

包含：
- 近 7 日订单趋势（每天多少单）
- 服务类型分布（保洁/废品/家政各占多少比例）
- 满意度占比（五星/四星/三星等占比）
- 时段分布（哪个时间段下单最多）
- 员工绩效排名（谁完成单量最多、评分最高）

**WorkBuddy干什么**
- 各统计聚合接口
- 返回数据格式适配 ECharts 图表组件入参
- 带时间范围筛选（本周/本月/自定义）

**人工干什么**
- ✅ 调用各聚合接口 → 返回的数字看起来合理（如总数 > 0、百分比加起来 = 100%）

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**⚠️ P2.11 完成后（即 P2 全部完成），AI 会自动生成 `Backend-API-Summary.md` 交接文档。**

**测试标准 — 通过后 P2 阶段完成**:
1. 各聚合接口返回数据格式符合 ECharts 入参要求
2. 数据来源于种子数据或前面单元创建的测试数据

---

### P3 居民端小程序（28h）

**目标**: 实现居民使用的微信小程序全部页面。这是用户量最大的端。

> **继续使用 DeepSeek V4 Pro 模型。**

---

#### P3.1 应用骨架 + 登录授权（3h）

**干什么**

小程序的"大门"：
- 启动时的隐私协议弹窗（首次使用必弹）
- 微信授权登录（静默获取用户 openid）
- 登录状态持久化（下次打开不用重新登录）
- 页面路由拦截（没登录不能进某些页面）

**WorkBuddy 干什么**
- 配置 App.vue（应用入口）
- 实现微信 wx.login 获取 code → 发送到后端换取 openid/token
- 隐私协议弹窗组件
- 路由守卫（拦截未登录访问）

**人工干什么**
- ✅ 微信开发者工具/H5 模式启动小程序
- ✅ 首次进入看到隐私协议弹窗
- ✅ 点同意后自动完成微信登录
- ✅ 关闭再重新打开 → 不再弹出协议（已同意过）

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 文件读写、Node.js、微信开发者工具

**测试标准 — 通过后方可进入 P3.2**:

---

#### P3.2 首页（3h）

**干什么**

小程序的主屏幕。类似 app 商店的首页——展示品牌信息和快捷入口。

包含：
- 品牌 Banner（大洋云洁 logo + 标语）
- 三大服务入口卡片（日常保洁 / 废品回收 / 家政咨询）
- 客服联系条
- 底部静态宣传图（资质证书、服务保障等）

**WorkBuddy 干什么**
- 首页布局和样式
- 三个服务卡片的点击跳转
- 静态内容填充

**人工干什么**
- ✅ 小程序首页能看到品牌 Banner
- ✅ 点击"日常保洁"跳转到保洁预约页（即使是空白的也行）
- ✅ 点击其他两个卡片同理

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P3.3**:
1. 各卡片点击能正确跳转到对应页面

---

#### P3.3 保洁预约三步向导（5h）

**干什么**

**居民端最复杂的功能**。用户预约保洁服务需要分三步完成：

```
第一步: 选择服务
  ├── 选择服务类型（日常保洁 / 深度保洁 / 开荒保洁）
  ├── 选择服务时长（2小时 / 3小时 / 4小时）
  └── 页面显示预估价格

第二步: 选择时间
  ├── 日历组件（选择日期）
  ├── 时段选择（上午/下午/晚上）
  └── 显示当日剩余可预约数

第三步: 确认下单
  ├── 信息汇总（服务+时间+地址+预估价）
  ├── 服务须知确认
  └── "提交订单" 按钮 → 成功页
```

**WorkBuddy 干什么**
- 先设计 Pinia store（三步之间传递数据的"中转站")
- 三个步骤页面各自的 UI 和逻辑
- 步骤间数据联动（选了服务类型，第二步的价格跟着变）
- 提交订单 → 调用后端 API → 成功页展示订单号

**人工干什么**
- ✅ **完整走通一遍**: 选服务 → 选时间 → 确认 → 看到 CLN 开头的订单号
- ✅ 测试返回修改（第三步返回第一步，数据保留）
- ✅ 测试不填必填项时提交（应有提示）

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P3.4**:
1. 完整走通三步提交，生成 CLN 前缀订单号
2. 返回上一步时已填数据保留
3. 必填项校验正常

---

#### P3.4 废品回收预约三步向导（4h）

**干什么**

类似保洁预约，但是针对废品回收：

```
第一步: 选择类型
  ├── 大件废品（家具/电器）
  └── 小件废品（纸箱/塑料瓶/金属）
  ├── 预估重量/数量

第二步: 选择时间（同保洁）

第三步: 确认下单（同保洁，生成 RCY 前缀订单号）
```

**WorkBuddy 干什么**
- 复用 P3.3 的向导框架
- 调整第一步的选项为废品相关
- 调用 RecyclingOrder 创建接口

**人工干什么**
- ✅ 完整走通 → 生成 RCY 开头订单号

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P3.5**:

---

#### P3.5 家政咨询流程（2h）

**干什么**

相对简单的咨询需求提交流程：

```
类型选择页 → 填写需求描述页 → 提交成功（CNS 前缀编号）
```

**WorkBuddy 干什么**
- 咨询类型选择页
- 需求填写表单（联系人、电话、详细诉求、期望时间）
- 提交 → 调用 ConsultOrder 创建接口

**人工干什么**
- ✅ 提交一份咨询 → 看到 CNS 编号

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P3.6**:

---

#### P3.6 我的订单—列表+详情（5h）

**干什么**

订单管理中心。用户查看所有订单的状态和详情。

包含：
- **Tab 切换**：保洁订单 / 废品回收（两个列表）
- **状态筛选**：全部 / 待派单 / 进行中 / 待评价 / 已完成
- **订单卡片**：订单号、服务类型、状态标签、时间、预估金额
- **订单详情页**：
  - 保洁订单详情：进度时间轴（每个状态节点的具体时间）+ 作业照片 + 评价入口
  - 废品订单详情：同上 + 实际重量 + 核定金额 + 验收状态

**WorkBuddy 干什么**
- 订单列表页（Tab + 筛选 + 卡片）
- 两种订单详情模板（保洁详情 / 废品详情，结构略有不同）
- 时间轴组件（展示订单生命周期各节点）
- 下拉刷新 + 上拉加载更多

**人工干什么**
- ✅ 列表页能看到之前测试创建的订单
- ✅ 点 Tab 能在保洁/废品间切换
- ✅ 点进一个保洁订单详情 → 看到时间轴
- ✅ 点进一个废品订单详情 → 看到额外重量/金额信息

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P3.7**:
1. 保洁和废品两种详情页均展示完整时间轴
2. 状态筛选正确过滤

---

#### P3.7 评价页 + 投诉页 + 我的页（3h）

**干什么**

三个独立的小功能页：

- **评价页**：星级选择 + 标签多选 + 文字输入 + 图片上传
- **投诉页**：投诉类型 + 描述 + 图片上传
- **我的页**：个人头像/昵称/电话 + 地址簿入口 + 客服电话 + 协议查看

**WorkBuddy 干什么**
- 评价表单页（star-rate 组件 + tag 选择 + textarea + 图片选择器）
- 投诉表单页
- 我的页面布局
- 地址簿增删改查（调用 P2.3 的地址 API）

**人工干什么**
- ✅ 提交一次评价（选 5 星、选标签、写字）
- ✅ 提交一次投诉
- ✅ "我的"页能看到个人信息
- ✅ 进地址簿 → 新增一个地址 → 回到列表能看到

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P3.8**:
1. 评价/投诉提交成功
2. 地址增删改查正常

---

#### P3.8 子女代下单（3h）

**干什么**

方便老年人使用的功能——子女可以帮老人下单。

在预约页面增加一个开关：「为自己下单 / 为家人下单」。选择"为家人下单"时需要填写老人的联系方式和服务对象的特殊需求。

**WorkBuddy 干什么**
- 预约页面增加代下单开关
- 开启后显示服务对象信息表单（姓名/电话/年龄/特殊注意事项）
- 提交订单时标记 isProxyOrder = true，附带老人信息

**人工干什么**
- ✅ 打开代下单开关 → 填写老人信息 → 提交 → 确认订单标记了代下单

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**⚠️ P3.8 完成后（即 P3 全部完成），AI 会自动生成 `MiniApp-Architecture.md` 交接文档。**

**测试标准 — 通过后 P3 阶段完成**:
1. 代下单订单 isProxyOrder = true，含老人信息
2. 微信开发者工具内完整走通「预约保洁 → 下单成功 → 查看订单详情 → 评价」闭环

---

### P4 员工端小程序（20h）

**目标**: 实现清洁员工使用的微信小程序。功能聚焦于"接单→干活→交工"的工作流。

> **继续使用 DeepSeek V4 Pro 模型。**

---

#### P4.1 应用骨架 + 员工身份绑定（2h）

**干什么**

员工端的启动和身份识别。与居民端类似但身份不同——登录后系统识别"你是哪个员工"。

**WorkBuddy 干什么**
- 复用 P3.1 的登录框架
- openid 匹配 Worker 表中的员工账号
- 绑定/解绑员工身份

**人工干什么**
- ✅ 员工微信登录后能看到自己的名字和工号

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P4.2**:

---

#### P4.2 首页—统计卡片 + 待服务列表（3h）

**干什么**

员工的工作台首页。一目了然今天要做什么：

- 今日统计：今日订单数 / 已完成数
- 待服务任务卡片列表（最近的待处理订单）
- 一键接单按钮

**WorkBuddy 干什么**
- 首页统计卡片（调后端聚合接口）
- 任务卡片列表
- 接单按钮（调后端 acceptOrder 接口）

**人工干什么**
- ✅ 首页数字显示正确（可与后端数据对比）
- ✅ 点击接单 → 卡片状态变化（变为"已接受"）

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P4.3**:

---

#### P4.3 我的任务列表（3h）

**干什么**

员工的全部任务视图：

- 双 Tab：保洁任务 / 废品回收任务
- 状态筛选胶囊：全部 / 待接单 / 已接受 / 服务中 / 已完成
- 任务卡片（订单号、服务地址、时间、状态）

**WorkBuddy 干什么**
- Tab 切换 + 状态筛选
- 任务卡片组件
- 列表数据拉取和渲染

**人工干什么**
- ✅ 切换 Tab → 列表内容变化
- ✅ 点击不同筛选条件 → 列表正确过滤

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P4.4**:
1. 按 Tab 和筛选条件正确过滤任务

---

#### P4.4 任务详情—待服务态（4h）

**干什么**

员工接到订单后看到的详情页面（订单还未开始服务时）。

展示：
- 客户信息（电话脱敏，如 138****1234）
- 订单信息（服务类型、时长、预约时间、地址）
- 进度时间轴（当前在哪一步）
- **「开始服务」按钮** → 触发 GPS 定位采集

**WorkBuddy 干什么**
- 任务详情页布局
- GPS 定位调用（wx.getLocation）
- 点击"开始服务" → 采集坐标 → 发送给后端 → 后端校验距离 → 状态变为 IN_SERVICE
- 距离超 200m 时的警告提示

**人工干什么**
- ✅ 查看任务详情能看到客户和订单信息
- ✅ 点击"开始服务" → 弹出位置授权 → 授权后状态变为"服务中"
  > 注：H5 模式下 GPS 可能用模拟坐标，真机测试时才准确

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P4.5**:
1. 点击开始服务触发 GPS 采集，状态变为 IN_SERVICE
2. 详情页展示完整的客户/订单信息

---

#### P4.5 任务详情—服务中态（3h）

**干什么**

同一任务详情页，但在"服务中"状态下的不同展示：

- **作业拍照**：前照（进门时）+ 后照（完工后），最少 2 张
- **服务指引弹窗**：弹出 SOP 话术（"您好，我是XX服务人员..."）+ 拍照清单提醒
  - 弹窗自动出现，10 秒倒计时后可关闭
- 「完成服务」按钮 → 上传照片 → 状态变为待评价

**WorkBuddy 干什么**
- 同一页面根据 status 条件渲染不同内容
- 相机/相册选择器（uni.chooseImage）
- SOP 弹窗组件（倒计时）
- 照片上传到 COS/本地存储
- 完成服务按钮及后续状态变更

**人工干什么**
- ✅ 进入服务中态的任务 → 看到 SOP 弹窗 → 10 秒后关闭
- ✅ 选择拍照 → 选 2 张图片 → 上传成功
- ✅ 点"完成服务" → 状态变为待评价

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P4.6**:
1. SOP 弹窗展示话术 + 拍照清单，倒计时后可关闭
2. 照片上传至存储
3. 完成后状态正确变更

---

#### P4.6 废品特殊流程 + 收款确认（3h）

**干什么**

废品回收订单在"服务中"之后的额外步骤：

- 员工录入实际重量
- 系统显示核定金额
- 等待居民在居民端小程序上验收
- 居民验收后，员工点「已收款」

**WorkBuddy 干什么**
- 重量录入表单（数字键盘）
- 核定金额展示（自动计算 = 重量 × 单价）
- 验收状态轮询/推送（等待居民验收）
- 「已收款」按钮（验收完成后才可用）

**人工干什么**
- ✅ 录入重量 15kg → 看到核定金额正确
- ✅ （另一端模拟）居民验收后 → 员工端显示"可收款"
- ✅ 点"已收款" → paymentStatus=PAID

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后方可进入 P4.7**:
1. 录入重量后等待居民验收
2. 收款后 paymentStatus=PAID

---

#### P4.7 我的页（2h）

**干什么**

员工的个人中心：

- 个人信息（姓名、工号、评分）
- 服务统计（总单量、本月单量、好评率）
- 证书查看（健康证、家政服务员证——图片展示）

**WorkBuddy 干什么**
- 个人信息展示
- 统计数据（调后端接口）
- 证书图片查看

**人工干什么**
- ✅ 能看到自己的信息、评分、统计数据
- ✅ 能查看证书图片

**使用模型**: **DeepSeek V4 Pro**

**需要权限**: 同上

**测试标准 — 通过后 P4 阶段完成**:
1. 所有信息展示正确
2. 完整走通「接单 → GPS 签到 → 拍照 → （废品录入重量+验收）→ 已收款」闭环

---

### P5 管理后台（24h）

**目标**: 实现物业管理人员使用的 Web 管理系统。用于处理订单、管理员工、查看数据。

> **此阶段可切换回 Auto 模型**（CRUD 模板化程度高），数据看板部分建议保持 DS V4 Pro。

---

#### P5.1 登录 + 布局框架（2h）

**干什么**

管理后台的门面：

- Admin 登录页面（账号密码登录）
- 登录后的整体布局（侧栏菜单 + 顶栏 + 内容区）
- 路由配置

**WorkBuddy 干什么**
- 登录页面（Element Plus 表单）
- 侧栏菜单组件
- 布局容器
- 路由守卫（未登录跳转登录页）

**人工干什么**
- ✅ 用 admin@dayunyunjie.com 登录 → 进入管理后台首页
- ✅ 侧栏菜单可见

**使用模型**: Auto

**需要权限**: 文件读写、Node.js、浏览器

**测试标准 — 通过后方可进入 P5.2**:

---

#### P5.2 数据看板（5h）

**干什么**

管理后台的首页——数据可视化大屏。让管理者一目了然地了解运营情况。

包含：
- **顶部 4 张统计卡**：今日订单 / 本周订单 / 活跃员工 / 满意度均值
- **订单趋势折线图**：近 7 天每天的订单量
- **服务类型环形图**：保洁/废品/家政占比
- **满意度环形图**：五星到一星的分布
- **时段柱状图**：一天 24 小时的订单分布热力
- **员工绩效表格**：排名列表（单量/评分/响应速度）

**WorkBuddy 干什么**
- ECharts 图表组件封装
- 6 个图表/表格的实现
- 调用 P2.11 的聚合 API 获取数据
- 时间范围切换（本周/本月）

**人工干什么**
- ✅ 打开管理后台首页 → 6 个图表/表格均有数据显示
- ✅ 数字看起来合理（不为空、不报错）
- ✅ 切换"本周"/"本月"数据刷新

**使用模型**: 建议 **DS V4 Pro**（ECharts 配置较复杂）

**需要权限**: 同上

**测试标准 — 通过后方可进入 P5.3**:
1. 各图表数据正确渲染
2. 切换时间范围数据刷新

---

#### P5.3 保洁订单管理（5h）

**干什么**

保洁订单的全生命周期管理界面：

- **列表页**：搜索框 + 状态 Tab（全部/待派单/进行中/...）+ 状态筛选 + 分页表格
- **新增弹窗**：手动创建订单（替居民下单的场景）
- **详情页/抽屉**：
  - 完整订单信息
  - 进度时间轴
  - 作业照片展示
  - 评价内容
  - 操作按钮（派单/取消等）

**WorkBuddy 干什么**
- Element Plus 表格（el-table）+ 分页（el-pagination）
- 搜索 + 筛选联动
- 新增订单弹窗（el-dialog + el-form）
- 订单详情抽屉（el-drawer）+ 时间轴组件
- 操作按钮调后端 API

**人工干什么**
- ✅ 列表能看到之前创建的测试订单
- ✅ 点击"新建" → 填写信息 → 创建成功
- ✅ 点击某行的"派单" → 选择员工 → 确认 → 订单状态变化
- ✅ 点进详情 → 看到完整时间轴和信息

**使用模型**: Auto 或 DS V4 Pro

**需要权限**: 同上

**测试标准 — 通过后方可进入 P5.4**:
1. 新增订单 → 分配员工 → 查看详情完整
2. 保洁订单全生命周期可在后台操作

---

#### P5.4 废品订单管理（3h）

**干什么**

与保洁订单管理结构基本一致，增加废品特有字段：

- 实际重量
- 核定金额
- 验收状态
- 收款状态

**WorkBuddy 干什么**
- 复用 P5.3 的表格/详情框架
- 增加废品特有字段的展示和编辑

**人工干什么**
- ✅ 废品订单列表和详情正常展示
- ✅ 能看到验收和收款相关状态

**使用模型**: Auto

**需要权限**: 同上

**测试标准 — 通过后方可进入 P5.5**:

---

#### P5.5 家政咨询单管理（2h）

**干什么**

简单的咨询单列表和详情：

- 列表（跟进状态筛选）
- 详情（联系信息 + 诉求描述）
- 状态变更按钮（待跟进 → 跟进中 → 已完成）

**WorkBuddy 干什么**
- 列表页 + 详情页
- 状态变更操作

**人工干什么**
- ✅ 咨询单列表可见
- ✅ 变更状态（待跟进 → 跟进中 → 已完成）流畅

**使用模型**: Auto

**需要权限**: 同上

**测试标准 — 通过后方可进入 P5.6**:

---

#### P5.6 员工管理（3h）

**干什么**

员工信息的 CRUD + 技能标签管理：

- 列表（状态筛选：在职/离职；技能筛选）
- 新增/编辑员工弹窗（姓名/电话/技能标签多选/证书上传）
- 证书管理（健康证/家政服务员证图片）

**WorkBuddy 干什么**
- 员工 CRUD 页面
- 技能标签多选组件（el-select multiple 或 el-checkbox-group）
- 证书图片上传和预览

**人工干什么**
- ✅ 新增一名员工 → 填写信息 → 选技能标签 → 保存 → 列表可见
- ✅ 编辑员工信息 → 修改技能 → 保存 → 变更生效

**使用模型**: Auto

**需要权限**: 同上

**测试标准 — 通过后方可进入 P5.7**:
1. 员工增删改查正常
2. 技能标签多选正常

---

#### P5.7 投诉反馈管理（2h）

**干什么**

投诉的处理和跟进：

- 列表（状态筛选）
- 详情（投诉内容 + 凭证图片）
- 处理录入 + 跟进记录 + 结案

**WorkBuddy 干什么**
- 投诉列表 + 详情
- 处理表单（处理意见、处理结果）
- 跟进记录时间线

**人工干什么**
- ✅ 查看投诉详情 → 录入处理意见 → 添加跟进 → 结案

**使用模型**: Auto

**需要权限**: 同上

**测试标准 — 通过后方可进入 P5.8**:
1. 投诉 → 处理中 → 已完成，跟进记录留存

---

#### P5.8 系统设置占位页（2h）

**干什么**

预留系统设置入口。一期只做占位页面（显示"功能开发中"），但菜单保留好以便二期扩展。

可能的设置项（二期再做）：
- 服务项目管理（增删改服务类型和价格）
- 公告管理
- 轮播图管理
- 系统参数配置

**WorkBuddy 干什么**
- 设置页面骨架
- 占位内容
- 菜单项配置

**人工干什么**
- ✅ 点击设置菜单 → 页面打开不报错 → 显示占位信息

**使用模型**: Auto

**需要权限**: 同上

**测试标准 — 通过后 P5 阶段完成**:
1. Chrome 全页面可操作，无 console 报错
2. 保洁订单全生命周期可在后台操作

---

### P6 集成与部署（12h）

**目标**: 把开发好的系统部署到线上环境，让真实用户可以使用。

> **切换回 Auto 模型。**

---

#### P6.1 WebSocket 实时推送（3h）

**干什么**

实现消息实时推送能力。当订单状态发生变化时，相关方立即收到通知，不用手动刷新页面。

例如：
- 管理后台派单 → 员工端小程序立即收到"您有新订单"提醒
- 员工完成服务 → 居民端小程序立即收到"服务已完成，请评价"提示

**WorkBuddy 干什么**
- Socket.IO 服务端集成
- 订单状态变更事件监听和广播
- 前端 WebSocket 连接和消息处理
- 管理后台实时刷新

**人工干什么**
- ✅ 后台派单后，观察员工端（或浏览器控制台）收到推送消息

**使用模型**: Auto

**需要权限**: 文件读写、终端

**测试标准 — 通过后方可进入 P6.2**:
1. 订单状态变更 → 管理后台实时刷新（不需 F5）

---

#### P6.2 微信订阅消息集成（3h）

**干什么**

配置微信小程序的订阅消息/模板消息，在关键业务节点主动推送通知给用户。

关键触达节点：
- 居民：派单通知、服务完成提醒、验收提醒、评价邀请
- 员工：新订单通知、订单取消通知

**WorkBuddy 干什么**
- 微信订阅消息 SDK 集成
- 消息模板配置
- 各业务节点的触发发送逻辑

**人工干什么**
- ✅ 需要在微信小程序后台申请消息模板（AI 会告诉你申请哪些模板）
- ✅ 测试：触发一个订单状态变更 → 手机/开发者工具收到消息通知

**使用模型**: Auto

**需要权限**: 文件读写、终端、微信小程序后台（需手动操作申请模板）

**测试标准 — 通过后方可进入 P6.3**:
1. 关键节点可推送消息

---

#### P6.3 Nginx 反向代理 + HTTPS（2h）

**干什么**

配置 Web 服务器，让用户可以通过域名安全地访问系统：

- Nginx 反向代理：将域名请求转发到 NestJS 后端和 Vue 前端
- HTTPS：SSL 证书配置，数据加密传输

**WorkBuddy 干什么**
- 生成 Nginx 配置文件
- 配置 API 反向代理（/api → NestJS :3000）
- 配置前端静态文件服务
- SSL 证书配置

**人工干什么**
- ✅ 将 Nginx 配置部署到服务器
- ✅ 申请域名和 SSL 证书（或在腾讯云一键申请）
- ✅ 浏览器访问 https://域名 正常打开

**使用模型**: Auto

**需要权限**: 文件读写、服务器 SSH 权限（或腾讯云控制台操作）

**测试标准 — 通过后方可进入 P6.4**:
1. HTTPS 访问 API + 管理后台正常

---

#### P6.4 PM2 进程守护 + 日志（2h）

**干什么**

确保后端服务稳定运行——崩溃了自动重启，日志妥善保存。

**WorkBuddy 干什么**
- PM2 配置文件（ecosystem.config.js）
- 进程守护配置（崩溃自动重启）
- 日志按日期分割（每天一个文件）

**人工干什么**
- ✅ 在服务器上安装 PM2
- ✅ 用 AI 生成的配置启动服务
- ✅ 手动 kill 进程 → 观察 PM2 是否自动重启

**使用模型**: Auto

**需要权限**: 终端、服务器 SSH

**测试标准 — 通过后方可进入 P6.5**:
1. 进程崩溃自动重启
2. 日志按日期分割

---

#### P6.5 全链路联调 + Bug 修复（2h）

**干什么**

最终的全面检验。在三端联调过程中发现并修复问题。

**WorkBuddy 干什么**
- 协助定位和修复联调中发现的问题
- 补充遗漏的功能细节
- 性能优化（如有明显问题）

**人工干什么**
- ✅ **完整走通以下核心场景**：
  1. 居民预约保洁 → 后台派单 → 员工接单 → GPS 签到 → 拍照完成 → 居民评价
  2. 居民预约废品回收 → 派单 → 接单 → 录入重量 → 居民验收 → 收款确认
  3. 管理后台查看数据看板 → 处理一笔投诉
- ✅ 发现任何问题都记录下来 → 反馈给 AI 修复 → 重新验证

**使用模型**: Auto

**需要权限**: 全部

**测试标准 — 通过后 🎉 整个项目一期 MVP 完成！**:
1. 居民-员工-后台全流程无阻塞性 bug
2. 生产环境部署完成

---

## 八、工时汇总与排期建议

### 总览

| 阶段 | 单元数 | 工时 | 模型 | 占比 |
|------|--------|------|------|------|
| P1 基础设施 | 5 个 | 16h | Auto | 10.7% |
| P2 后端核心 API | 16 个 | 40h | Auto + DS V4 Pro | 26.7% |
| P3 居民端小程序 | 8 个 | 28h | DS V4 Pro | 18.7% |
| P4 员工端小程序 | 7 个 | 20h | DS V4 Pro | 13.3% |
| P5 管理后台 | 8 个 | 24h | Auto / DS V4 Pro | 16.0% |
| P6 集成与部署 | 5 个 | 12h | Auto | 8.0% |
| **合计** | **49 个** | **140h** | — | **100%** |

### 排期建议

假设每天有效开发 **6-8 小时**（含 AI 等待 + 人工审查 + 测试时间）：

| 方案 | 每天工时 | 预计工作日 | 预计日历天数 |
|------|---------|-----------|-------------|
| 轻松节奏 | 6h | ~23 天 | 约 1 个月（扣除周末） |
| 正常节奏 | 8h | ~18 天 | 约 3-4 周（扣除周末） |
| 加快节奏 | 10h | ~14 天 | 约 2-3 周（扣除周末） |

### 建议里程碑

| 日期节点 | 目标 |
|----------|------|
| **第 1 周** | 完成 P1 + P2.1-P2.4（基础设施 + 认证 + 基础 CRUD），项目骨架跑通 |
| **第 2 周** | 完成 P2.5-P2.6（订单状态机），核心业务逻辑完成 |
| **第 3 周** | 完成 P2.7-P2.11 + P3（后端收尾 + 居民端小程序） |
| **第 4 周** | 完成 P4 + P5（员工端 + 管理后台） |
| **第 5 周** | 完成 P6（集成部署），MVP 上线 |

---

## 九、风险与应急处理

### 最大风险及应对

| 风险 | 影响 | 概率 | 应对措施 |
|------|------|------|----------|
| **订单状态机逻辑缺陷** | P2.5/P2.6 延迟阻塞后续开发 | 中 | 用 DS V4 Pro 生成后立即人工 review 转移规则表；让 AI 生成全套测试用例逐一验证 |
| **微信登录依赖真机/AppID** | P3/P4 部分功能无法在模拟器中测试 | 高 | 先 mock openid 开发全部页面逻辑；后期统一切换真实微信环境，只需改配置 |
| **COS 配置依赖腾讯云账号** | 拍照功能无法真测 | 低 | 开发期用本地 `/uploads` 存储；部署前一行配置切换 COS |
| **uni-app H5 与微信小程序差异** | 部分功能 H5 能跑但微信不行 | 中 | 核心流程必须在微信开发者工具中验证；GPS/拍照等原生能力尤其注意 |
| **单个开发单元 AI 产出质量不达标** | 需要多轮修复影响进度 | 低 | 每个单元严格按测试标准验收；不通过不进入下一单元 |

### 出问题了怎么办

```
┌─────────────────────────────────────────────────────┐
│                                                     │
│  AI 生成的代码有问题？                                │
│                                                     │
│  ① 直接用中文描述问题                                │
│     "订单列表不显示数据"                              │
│     "登录后跳转到错误页面"                            │
│     "派单后状态没有变成已派单"                        │
│                                                     │
│  ② AI 会尝试修复                                    │
│                                                     │
│  ③ 修复后再测试                                     │
│                                                     │
│  ④ 还不行？继续反馈，或者：                           │
│     → 让 AI 解释它在做什么（帮助理解问题根因）        │
│     → 让 AI 回退到上一个可用版本（git revert）        │
│     → 跳过这个单元先做后面的（如果无依赖关系）         │
│                                                     │
└─────────────────────────────────────────────────────┘
```

### Git 安全网

**每个开发单元完成后，执行一次 git commit。** 这是最重要的安全措施：

- 如果 AI 改坏了东西，随时可以回到上一个正常的版本
- commit 信息由 AI 自动生成，你只需要确认

**如何回滚（如果需要）**：
> 对 AI 说："回滚到上一个版本" 或 "回滚到 P2.5a 完成时的版本"

---

## 附录 A：快速启动口令

开始每个开发单元时，直接复制以下口令发给 WorkBuddy 即可：

| 单元 | 启动口令 |
|------|----------|
| P1.1 | "开始 P1.1，初始化 NestJS 项目脚手架和 Prisma" |
| P1.2 | "开始 P1.2，将 Schema.md 转译为 Prisma schema 文件" |
| P1.3 | "开始 P1.3，创建共享类型包 packages/shared" |
| P1.4 | "开始 P1.4，创建 uni-app 双端（居民+员工）项目骨架" |
| P1.5 | "开始 P1.5，基于 youlai 模板创建 Vue 3 管理后台脚手架" |
| P2.1 | "开始 P2.1，实现 Auth 模块（微信登录 + JWT）" |
| P2.2 | "开始 P2.2，实现三类用户的 CRUD 模块" |
| P2.3 | "开始 P2.3，实现地址管理模块" |
| P2.4 | "开始 P2.4，实现服务目录查询模块" |
| P2.5a | "开始 P2.5a，实现 CleaningOrder CRUD 和创建订单" |
| P2.5b | "开始 P2.5b，实现保洁订单状态机核心逻辑" |
| P2.5c | "开始 P2.5c，实现派单/GPS签到/完成/取消操作接口" |
| P2.6a | "开始 P2.6a，实现 RecyclingOrder CRUD 和继承状态机" |
| P2.6b | "开始 P2.6b，实现废品特有流程（验收+重量+收款）" |
| P2.7 | "开始 P2.7，实现 ConsultOrder 咨询单模块" |
| P2.8 | "开始 P2.8，实现 GPS 签到校验服务" |
| P2.9 | "开始 P2.9，实现 COS 文件上传和水印功能" |
| P2.10 | "开始 P2.10，实现评价与投诉模块" |
| P2.11 | "开始 P2.11，实现数据看板聚合 API" |
| P3.1 | "开始 P3.1，实现居民端应用骨架和登录授权" |
| P3.2 | "开始 P3.2，实现居民端首页" |
| P3.3 | "开始 P3.3，实现保洁预约三步向导" |
| P3.4 | "开始 P3.4，实现废品回收预约三步向导" |
| P3.5 | "开始 P3.5，实现家政咨询提交流程" |
| P3.6 | "开始 P3.6，实现我的订单列表和详情页" |
| P3.7 | "开始 P3.7，实现评价页、投诉页和我的页" |
| P3.8 | "开始 P3.8，实现子女代下单功能" |
| P4.1 | "开始 P4.1，实现员工端应用骨架和身份绑定" |
| P4.2 | "开始 P4.2，实现员工端首页（统计+待服务列表）" |
| P4.3 | "开始 P4.3，实现员工端任务列表（双Tab+筛选）" |
| P4.4 | "开始 P4.4，实现任务详情—待服务态（含GPS签到）" |
| P4.5 | "开始 P4.5，实现任务详情—服务中态（拍照+SOP弹窗）" |
| P4.6 | "开始 P4.6，实现废品特殊流程和收款确认" |
| P4.7 | "开始 P4.7，实现员工端我的页" |
| P5.1 | "开始 P5.1，实现管理后台登录和布局框架" |
| P5.2 | "开始 P5.2，实现数据看板（6个图表）" |
| P5.3 | "开始 P5.3，实现保洁订单管理（列表+新增+详情）" |
| P5.4 | "开始 P5.4，实现废品订单管理" |
| P5.5 | "开始 P5.5，实现家政咨询单管理" |
| P5.6 | "开始 P5.6，实现员工管理（CRUD+技能标签）" |
| P5.7 | "开始 P5.7，实现投诉反馈管理" |
| P5.8 | "开始 P5.8，实现系统设置占位页" |
| P6.1 | "开始 P6.1，实现 WebSocket 实时推送" |
| P6.2 | "开始 P6.2，集成微信订阅消息" |
| P6.3 | "开始 P6.3，配置 Nginx 反向代理和 HTTPS" |
| P6.4 | "开始 P6.4，配置 PM2 进程守护和日志" |
| P6.5 | "开始 P6.5，全链路联调和 Bug 修复" |

---

## 附录 B：术语速查表

| 术语 | 通俗解释 |
|------|----------|
| API | 接口。小程序和后端对话的"窗口"，比如"创建订单"就是一个 API |
| CRUD | 增删改查。Create(新建)、Read(读取)、Update(修改)、Delete(删除) |
| DTO | 数据传输对象。规定 API 接收和返回的数据格式 |
| Schema | 数据库表结构定义。描述一张表有哪些列、各列是什么类型 |
| Migrate | 数据迁移。把 Schema 的变更同步到数据库（比如加了一列，数据库也加上） |
| Seed | 种子数据。预先填入数据库的测试数据 |
| JWT | JSON Web Token。一种用户登录凭证，类似"临时身份证" |
| Guard | 门卫。后端的安检机制，没带有效凭证的请求会被拦住 |
| Swagger | API 文档工具。自动生成可在线测试的接口文档页面 |
| State Machine | 状态机。规定订单可以从 A 状态变到 B 状态，但不能跳到 C 状态 |
| ECharts | 图表库。用来画折线图、饼图、柱状图等的工具 |
| COS | 腾讯云对象存储。云端硬盘，用来存照片等文件 |
| Nginx | Web 服务器。把域名请求转发给后端程序 |
| PM2 | 进程管理器。保证后端程序一直运行，崩了自动重启 |
| uni-app | 一个框架。写一套代码，可以同时编译成微信小程序和 H5 网页 |
| Pinia | 状态管理。小程序里在不同页面之间共享数据（比如购物车）的工具 |
| Composition API | Vue 3 的一种写代码的方式（另一种是 Options API） |
| Element Plus | 基于 Vue 3 的 UI 组件库（按钮、表格、弹窗等现成组件） |
| Haversine | 根据两个经纬度坐标计算地球表面距离的数学公式 |
| watermark | 水印。在图片上叠加的文字或图案（防止盗用） |
| endpoint | API 的具体地址，比如 POST /api/orders |
| e2e | End-to-End。端到端测试，从头到尾完整跑一遍流程 |

---

> **文档版本**: v1.0
> **创建日期**: 2026-06-01
> **适用范围**: 大洋云洁 (dayangyunjie) 社区服务平台一期 MVP
> **使用方式**: 按 P1 → P2 → P3 → P4 → P5 → P6 顺序，逐单元执行。每单元完成后按"测试标准"验收，通过后进入下一单元。
