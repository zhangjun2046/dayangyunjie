...
| Nginx | 新增 `/m-admin/` 静态站点（见 §4.6） | **必须（首次）** |
...
| 管理端 H5 | `http://118.195.149.50/m-admin/` |
| API | `http://118.195.149.50/api/v1/...`（两端共用） |

H5 使用 **hash 路由**，登录页实际为：  
`http://118.195.149.50/m-admin/#/pages/login/index`  
...
- [ ] 生产构建 `base` 为 `/m-admin/`（`apps/miniapp-admin/vite.config.ts`：`NODE_ENV === 'production'` 时生效）
...
校验产物里的资源路径应带 `/m-admin/` 前缀（例如 `index.html` 中的 `src="/m-admin/assets/..."`）。若仍是根路径 `/assets/...`，说明构建未带上 `base`，需确认代码已 pull 到含 `vite.config.ts` base 配置的版本后重构建。

### 4.6 修改 Nginx（首次必做）

编辑 `/etc/nginx/conf.d/dayangyunjie.conf`，在**保留**现有 `/`、`/api/`、`/uploads/` 的基础上，**新增** `/m-admin/`。完整参考如下（可直接整文件替换，替换前请确认 §3 已备份）：
...
server {
    ...
    location /m-admin/ {
        ...
        try_files $uri $uri/ /m-admin/index.html;
    }
...
>   `location /m-admin/ { alias /var/www/dayangyunjie-miniapp-admin/; index index.html; }`
...
| # | 操作 | 期望 |
|---|------|------|
| 1 | 浏览器打开 `http://118.195.149.50/m-admin/` | 出现登录页（非 PC 后台、非 404） |
| 2 | Admin 邮箱+密码登录 | 进入订单列表 |
| 3 | 切换保洁/废品 Tab、下拉刷新 | 列表正常；接口走同域 `/api/v1` |
| 4 | 打开待派单 → 分配/改派 | 可选到在职空闲员工；离职员工不出现 |
| 5 | 打开订单详情 → 返回 | 能回到列表（hash 路由） |
| 6 | 强刷 `http://118.195.149.50/m-admin/#/pages/orders/index` | 不 404 |

快速 curl：

```bash
curl -sI http://118.195.149.50/m-admin/
...
| 层级 | 做法 |
|------|------|
| 管理端 H5 | 删除或注释 Nginx 中 `/m-admin/` 段 → `nginx -t && systemctl reload nginx`；可选清空 `/var/www/dayangyunjie-miniapp-admin/` |
...
| 打开 `/m-admin/` 404 | Nginx 未改或未 reload | 检查 §4.6；`nginx -t` |
| `/m-admin/` 白屏，控制台 JS 404 | 构建 `base` 未带 `/m-admin/`，或静态未拷全 | 确认 vite base → 重做 §4.5 |
| `/m-admin/` 打开却是 PC 后台 | `location /` 抢先匹配或 alias 写错 | 确认 `location /m-admin/` 在配置中且 `alias` 指向 miniapp 目录 |
