...
| P5.6 服务人员管理                           | ✅ 已通过 | 2026-06-23 | 管理端 `/workers` 完整实现：列表（今日订单列 + 重置密码 + 技能单选筛选）+ 新增/编辑弹窗（v2.0 扩展字段 + 证书上传）+ 详情抽屉（证书区 + 绩效统计无创收金额 + 投诉记录列表）；后端扩展 `skillType` 筛选、`todayOrders` 聚合、`GET /complaints?workerId=`；修复：手机号搜索、CreateWorkerDto 白名单 400、健康证/技能证书改非必填；使用 Sonnet 4.6 LLM 完成 |
| P5.7 投诉反馈管理                           | ✅ 已通过 | 2026-06-23 | 管理端 `/orders/complaint` 完整实现：列表（关联订单列 + 投诉内容列 + 客户联系方式筛选；移除服务时间/服务人员/摘要列）+ 详情抽屉（关联原始订单 + 投诉内容/凭证图 + 跟进时间轴 + 提交/结束按钮）；后端 `findAll`/`findOne` 关联订单富 DTO + `keyword`/`contactPhone` 查询；修复：keyword 400、客户姓名误匹配投诉单号子串；使用 Sonnet 4.6 LLM 完成 |
...
| P5.12 管理后台首页（工作台）                | ✅ 已通过 | 2026-07-19 | 替代 `/dashboard` 长期停留的 P1.5 脚手架占位页：改写 `apps/admin/src/views/dashboard/index.vue` 为欢迎条（姓名+角色+问候语）+ 4 张按功能授权过滤的待办事项卡片（保洁待派单/废品待派单/家政待跟进/投诉待处理）；卡片数字复用 P5.3/P5.4/P5.5/P5.7 列表接口 `pageSize=1` 的 `total` 字段，**无新增后端接口**；按 `useUserStore().hasMenu(menuKey)` 并发触发可见卡片请求，超级管理员始终展示全部四张；点击卡片 `router.push` 跳转对应列表页并带 `status=` 查询参数，4 个订单列表页 `onMounted` 新增读取 `route.query.status` 回填 `queryParams.status` 预置筛选逻辑；`markRaw` 包裹图标组件避免 Vue 响应式警告；通过浏览器验证：超级管理员登录 4 张卡片数字与列表页条数一致（cleaning=1/recycling=0/consult=7/complaint=2）、仅授权 `orders.cleaning` 账号登录仅展示 1 张卡片、4 张卡片点击跳转均正确预置状态筛选（Tab 高亮 + 列表已过滤）；`npm run build`（shared + admin）通过；使用 Sonnet 5 LLM 完成 |
| P-UI Icon 替换（居民端 + 员工端）           | ✅ 已通过 | 2026-07-19 | 居民端/员工端静态图标全面替换为 PNG（tabBar + 业务图标）；SVG 业务图标下线；调整自带底色徽标 PNG 与外层容器尺寸匹配（外圈过大、内 icon 过小问题）；员工端废品订单列表 `serviceName` 字段修正为读 `serviceItem`（原误读 `serviceType` 导致小类空白）；清理调试用 `fetch(127.0.0.1)` 埋点避免小程序 `fetch is not a function`；使用 Sonnet 5 LLM 完成 |
| P-微信 真实登录接入（居民端 + 后端）        | ✅ 已通过 | 2026-08-07 | 后端新增 `auth/wechat-customer.service.ts`，接入真实 `code2session` + `getuserphonenumber`（配置 `WECHAT_CUSTOMER_APPID`/`SECRET` 时生效，未配置回落 mock）；`auth.service`/`controller`/`module`/`spec`、`dto/auth-response`、`dto/decrypt-phone`、`common/config/env-config.service` 同步改造；居民端 `api/auth.ts`、`store/auth.ts`、`PrivacyModal`、`ProfileCompleteModal`、`useRouteGuard` 改造，新增 `utils/wechat-privacy.ts` 与 `pages/agreement/`；首页改为先登录再授权手机号避免 `/auth/decrypt-phone` 401；同微信号重登按稳定 openid 找回历史订单；新增 `docs/Wechat-Enterprise-Launch-Audit.md`；**接口契约变更（接手复查补记）**：`POST /auth/decrypt-phone` 增加 `@UseGuards(JwtAuthGuard)`，未带 token 一律 401，且解密结果通过 `resident.update` 写回 `resident.phone`（不再仅返回值），`ResidentProfileDto` 相应新增 `phone`（`wechatLogin` / `getProfile` 均返回）；`docs/Backend-API-Summary.md` 与 `docs/OrderModule-API-Contract.md` 本次未同步，需后续补；**本地联调开关**：`hasWechatCustomerCredentials` 要求 APPID 与 SECRET 同时存在，本机 `apps/server/.env` 两者齐全时后端直连微信，Swagger/curl 用假 code 调 `wechat-login` 会被微信 40029 拒绝，需 mock 时注释掉 `WECHAT_CUSTOMER_SECRET` |
| P-交接 员工端与配套能力补充                 | 🔄 待验收 | —          | 员工端新增 `pages/change-password/`（替代旧 `pages/settings/` 内联折叠表单）、`pages/agreement/`、`src/components/`；三端客服/接单人电话能力（`ContactOperatorPicker.vue` + `utils/call-contact-operator.ts` + 员工端 `api/operator.ts` + 后端 `operator` 模块）；居民端新增 `pages/address-edit/`；订单完成流程调整（保洁/废品 `complete-order.dto` + controller/service + `packages/shared/entities/order.ts` + 管理端 `api/cleaning`/`api/recycling`/数据看板）；UI 素材全量替换（居民端 6 tab + 18 业务图标，员工端 6 tab + 10 业务图标）；新增 `scripts/link-uni-local-deps.mjs` 打通 HBuilderX 编译；**顺带修复的真 bug（接手复查补记）**：`auth.service.refreshToken` 原先不分角色统一查 `resident` 表，员工端 / 管理端刷新 token 必然抛 `Unauthorized`，现按 `payload.role` 分流——`AUTH_ROLE_WORKER` 查 `worker` 后 `issueWorkerTokens`，`AUTH_ROLE_ADMIN` 查 `admin` 并校验 `status=ENABLED` 后 `issueAdminTokens`，员工端 `store/auth.ts` 同步改造（112 行）；**Windows 接手注意**：两个小程序 `package.json` 新增 `postinstall` 会在每次 `npm install` 自动执行该脚本，其 `linkPath()` 用 `fs.symlinkSync` 建软链，Windows 下创建符号链接需管理员权限或开启系统「开发者模式」，否则报 `EPERM` 且脚本 `process.exit(1)` 会让 `npm install` 整体失败（改 `junction` 亦可，但 junction 不支持相对目标，须先 `path.resolve` 成绝对路径）；脚本中 `dedupeUniCliShared()` 并非 mac 专用——它解决 npm 嵌套多份 `uni-cli-shared` 导致 easycom 组件表实例不一致、`uni-nav-bar`/`uni-icons` 进不了 `usingComponents` 的问题，Windows 同样需要，**不可直接删除 postinstall 图省事** |

> **P2.1–P2.15 后端核心 API 全部完成（含 v2.0 补充）。P3.1–P3.8 居民端全部完成。P4.1–P4.7 员工端小程序全部完成。** **P5.1 管理后台登录与布局框架已完成。P5.2 数据看板已完成。P5.3 保洁订单管理已完成。P5.4 废品订单管理已完成。P5.5 家政咨询单管理已完成。P5.6 服务人员管理已完成。P5.7 投诉反馈管理已完成。P5.8 系统管理-用户管理已完成。P5.8b 功能授权已完成。P5.9 服务配置管理已完成。P5.10 运营人员信息配置已完成。P5.11 轮播图管理已完成。P5.12 管理后台首页（工作台）已完成。P-UI Icon 替换（居民端+员工端）已完成。** **P5 阶段全部完成**；P6 集成与部署待启动。
...
**P5.7 验收结果（2026-06-23）**：✅ 已通过。列表含关联订单/投诉内容列，无服务时间/服务人员列；详情抽屉跟进时间轴 + 提交/结束结案流程正常；关键词与客户联系方式查询可用。
...
> **验收状态**：✅ **已通过**（2026-07-06）  
> **验收依据**：① 管理端 `/config/operators` 列表（姓名/手机号完整展示/用途/创建时间）+ 姓名手机号 keyword 搜索 + 新增/编辑弹窗 + 删除确认；② 后端 `QueryOperatorDto` 扩展 `name`/`phone`/`keyword` 模糊查询；③ `apps/admin/src/api/operator.ts` 新建 CRUD；④ `npm run build` 通过；使用默认 LLM 完成。  
> 下一单元：[P5.11](#p511-轮播图管理2h)  
...
> **验收状态**：✅ **已通过**（2026-07-06）  
> **验收依据**：① 管理端 `/config/banners` 列表 + 筛选（状态/展示端/标题）+ 分页 + 删除确认；② 新增/编辑弹窗三块表单（基础信息/展示配置/跳转配置）+ 图片上传 + 生效时间范围；③ 后端 `QueryBannerDto.title` 模糊查询 + `CreateBannerDto.isEnabled`；④ `imageUrl` 校验 `require_tld: false` 兼容本地 `http://localhost:3000/uploads/`；⑤ `banner.spec` 13 项通过；⑥ `npm run build` 通过；使用默认 LLM 完成。  
> 下一单元：[P6.1](#p61-websocket-实时推送3h)  
...
#### P5.12 管理后台首页（工作台）（2h）✅ **已通过**

> **需求来源**：`requirement_v2.0.md` §5.6、§10.2 #105–#107（2026-07-19 补充需求）  
> **背景**：`/dashboard`（登录后默认落地页，不在侧栏菜单、不受功能授权限制）长期停留在 P1.5 脚手架占位文案（"当前为 P1.5 脚手架阶段…"），此前需求文档从未定义过它的具体内容，是需求空白点而非漏做的已定义单元。本单元完成后 P5 阶段才算真正全部收尾。

**干什么**

把首页从占位文案改为「工作台」：登录后第一眼看到今天有哪些事要处理，而不是重复数据看板（`/data/dashboard`，P5.2 已完成）的经营统计图表。

- **欢迎条**：当前登录用户姓名 + 角色（超级管理员/普通管理员）+ 问候语（复用登录态已有信息，无需新接口）
- **待办事项卡片组**（核心）：4 张卡片——待派单保洁订单数 / 待派单废品订单数 / 待跟进家政咨询单数 / 待处理投诉数；每张展示数字徽章，点击跳转对应列表页并预置状态筛选
- **权限过滤**：卡片按登录用户的功能授权（P5.8b）动态显示/隐藏，逻辑与侧栏菜单一致（无 `orders.cleaning` 权限则不展示「待派单保洁」卡片，其余三张同理）；超级管理员始终展示全部四张
- **不做**：图表/统计趋势（避免与 P5.2 重复维护）、系统公告（系统当前无此模块，需要的话应单独立项）、快捷操作磁贴、自定义布局

**Cursor Agent 干什么**

- 改写 `apps/admin/src/views/dashboard/index.vue`，替换 P1.5 占位内容
- 待办卡片数据源全部复用已有列表接口，用 `pageSize=1` 取分页 `total` 即可，**无需新增后端接口**：
  - `GET /cleaning-orders?status=PENDING_ASSIGN&page=1&pageSize=1`
  - `GET /recycling-orders?status=PENDING_ASSIGN&page=1&pageSize=1`
  - `GET /consult-orders?status=FOLLOW_UP&page=1&pageSize=1`
  - `GET /complaints?status=PENDING&page=1&pageSize=1`
- 四个请求并发触发，按 `useUserStore().hasMenu(menuKey)` 决定是否发起对应请求/展示对应卡片
- 点击卡片跳转到对应列表页并带上预置状态筛选参数（如 `/orders/cleaning?status=PENDING_ASSIGN`）

**人工干什么**

- ✅ 登录后默认进入首页，看到欢迎条（姓名+角色）
- ✅ 超级管理员账号：4 张待办卡片全部展示，数字与对应列表页「待派单/待跟进/待处理」条数一致
- ✅ 仅授权 `orders.cleaning` 的账号登录：首页只展示「待派单保洁订单」卡片，其余三张不显示
- ✅ 点击任意卡片正确跳转到对应列表页并已按状态筛选
...
1. 首页不再显示 P1.5 占位文案 ✅
2. 待办卡片数字与对应列表页条数一致 ✅
3. 待办卡片按登录用户功能授权正确显示/隐藏 ✅
4. 点击卡片跳转并预置筛选正确 ✅

**P5.12 验收记录（2026-07-19）**：

- 超级管理员登录：4 张待办卡片全部展示，数字分别为 保洁=1 / 废品=0 / 家政=7 / 投诉=2，与对应列表页在同一状态筛选下的条数一致
- 仅授权 `orders.cleaning` 权限的测试账号登录：首页仅展示「待派单保洁订单」1 张卡片，其余三张不显示
- 依次点击 4 张卡片，均正确跳转到 `/orders/cleaning`、`/orders/recycling`、`/orders/consult`、`/orders/complaint`，并通过 `status=` 查询参数预置状态筛选（对应 Tab 高亮 + 列表按该状态过滤）
- `npm run build`（`@dayangyunjie/shared` + `@dayangyunjie/admin`，含 `vue-tsc --noEmit`）通过，无新增 TypeScript/ESLint 报错
- **P5 阶段（管理后台）全部单元（P5.1–P5.12）验收通过，进入 P6 集成与部署阶段**
...
| P5 管理后台（含配置管理+系统管理+首页） | 13 个     | 42h      | 默认/Fast + 强模型 | 26.3%    |
| P6 集成与部署              | 5 个      | 12h      | 默认/Fast          | 7.5%     |
| **v2.0 合计**              | **53 个** | **160h** | —                  | **100%** |
...
| P5.12 | "开始 P5.12（v2.0），实现管理后台首页/工作台（欢迎条 + 4 张按功能授权过滤的待办事项卡片，复用已有列表接口 total，无需新增后端接口）"           |
...
> **文档版本**: v4.7
> **创建日期**: 2026-06-01
> **修订日期**: 2026-07-19（v4.7：P5.12 管理后台首页/工作台验收通过——`/dashboard` 改写为欢迎条 + 4 张权限过滤待办卡片，复用已有列表接口 `total`，无新增后端接口；**P5 阶段全部收尾** | v4.6：新增 P5.12 单元——需求已随 `requirement_v2.0.md` §5.6 补充冻结 | v4.5：P5.11 轮播图管理验收通过 | v4.4：P5.10 运营人员信息配置验收通过 | v4.3：P5.7 投诉反馈管理验收通过 | v4.2：P5.6 服务人员管理验收通过 | v4.1：P5.5 家政咨询单管理验收通过 | v4.0：P5.4 废品订单管理验收通过 | v3.9：P5.3 保洁订单管理验收通过 | v3.8：P5.1 管理后台登录+二级折叠菜单布局验收通过 | v3.7：P4.7 员工端我的页验收通过（技能证书+修改密码+无服务记录入口） | v3.6：P4.6 任务详情待评价/已完成态 | v3.5：P4.5 任务详情服务中态 | v3.4：P4.4 任务详情已派单/已接单态 | v3.3：P4.3 任务列表 | v3.2：P4.2 首页 | v3.1：P4.2 进度表 | v3.0：P4.1 登录）
> **适用范围**: 大洋云洁 (dayangyunjie-code) 社区服务平台一期 MVP（v2.0 基线）
> **使用方式**: 按 P1 → P2 → P3 → P4 → P5 → P6 顺序，逐单元执行。每单元完成后按"测试标准"验收，通过后进入下一单元。P2.1–P2.15 已完成（v1.x + v2.0 阶段）；P3.1–P3.8 已全部完成（含代下单集成验证）；P4.1–P4.7 员工端小程序全部完成；**P5.1–P5.12 管理后台全部完成，P5 阶段收尾**；P6 集成与部署待启动。
