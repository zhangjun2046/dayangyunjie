...
> **P2.1–P2.15 后端核心 API 全部完成（含 v2.0 补充）。P3.1–P3.8 居民端全部完成。P4.1 员工端登录认证已完成。** 下一阶段：**P4.2** — 员工端首页待接单任务列表。
...
- 客户信息：姓名（示例：张女士）、脱敏手机号、一键拨号 + 地图导航
- ✏️ 代下单场景：额外展示「被服务人」姓名 + 脱敏手机号
- 服务进度时间轴（精确显示当前节点）
- ✏️ 作业记录区（上传前后照片按钮）：**ACCEPTED 状态下灰色禁用态**，仅 IN_SERVICE 后可操作
- 底部主按钮：**「开始服务」** → 触发 GPS 签到（调 `/cleaning-orders/:id/gps-checkin` 或废品同名接口）→ 状态变 IN_SERVICE
- GPS 超距（>200m）：显示警告并要求填写说明

**Cursor Agent 干什么**

- 任务详情页（多状态条件渲染）
- GPS 定位（wx.getLocation）→ 坐标发送到后端
- 作业记录区按 status 控制 enabled/disabled 状态
- 代下单被服务人信息展示

**人工干什么**

- ✅ ASSIGNED 状态：「开始服务」可点；作业照片区禁用
- ✅ 点「开始服务」→ GPS 授权 → 状态变 IN_SERVICE → 作业照片区解锁
- ✅ 代下单订单展示被服务人信息
...
1. 开始服务触发 GPS 采集，状态变 IN_SERVICE
2. ACCEPTED 状态下作业照片区为灰色禁用
3. IN_SERVICE 后作业照片区解锁可上传
...
- 上门日期 / ✏️ **服务时段**（上午 08:00–11:00 / 下午 14:00–17:00）
...
| P4.5  | "开始 P4.5，实现任务详情—服务中态（无SOP弹窗 + 保洁「完成服务」按钮 + 废品无完成按钮 + 无重量/金额字段）"                                      |
...
> **文档版本**: v3.0
> **创建日期**: 2026-06-01
> **修订日期**: 2026-06-21（v3.0：P4.1 员工端登录页+身份认证验收通过 | v2.9：P3.8 代下单集成验证通过，生成 MiniApp-Architecture.md | v2.8：P3.6_repair 废品验收改为员工触发与保洁对称 | v2.7：P3.7 评价页+投诉页+我的页验收通过 | v2.6：P3.6 验收通过并补充 mp-weixin onLoad 兼容修复 + scroll-view 内联胶囊修复 | v2.5：P3.6 我的订单列表+详情页验收通过 | v2.4：P3.5 家政咨询提交流程验收通过 | v2.3：P3.3 + P3.4 预约向导验收通过 | v2.2：P3.2 首页验收通过 | v2.1：P3.1 骨架+登录验收通过 | v2.0：基于需求文档 v2.0 全面更新 | v1.3：废品流程调整 | v1.2：WorkBuddy → Cursor Agent）
> **适用范围**: 大洋云洁 (dayangyunjie-code) 社区服务平台一期 MVP（v2.0 基线）
> **使用方式**: 按 P1 → P2 → P3 → P4 → P5 → P6 顺序，逐单元执行。每单元完成后按"测试标准"验收，通过后进入下一单元。P2.1–P2.15 已完成（v1.x + v2.0 阶段）；P3.1–P3.8 已全部完成（含代下单集成验证）；P4.1 员工端登录认证已完成，当前进入 P4.2 员工端首页。
