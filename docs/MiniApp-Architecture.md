...
### 18.6 P5.4 废品订单管理（已完成）

`views/orders/recycling/index.vue` 已实现完整废品订单管理（复用 P5.3 框架）：

- **列表页**：订单编号 / 客户信息 / 被服务人 / 被服务人联系方式 / 是否代下单 / 服务类型 / **预估重量** / 服务地址 / 服务时间 / 服务人员 / 状态 / 操作（**无金额列**）
- **筛选**：关键词 + 客户联系方式 + 服务地址 + 状态 Tab（与保洁一致，无预约日期范围）
- **分配弹窗**：订单摘要 + 服务人员下拉 → `POST /recycling-orders/:id/assign`
- **新增订单弹窗**：RECYCLING 服务目录 + 预估重量步进（5–200kg，步长 5）+ 代下单 + 8 格起始时间点 + 文本地址代下单
- **详情抽屉**：展示预估重量、服务人员、时间轴、作业照片、用户评价；**不展示** actualWeight / 核定金额 / 收款状态
- **API 封装**：`apps/admin/src/api/recycling.ts`
...
### 18.8 P5.6 服务人员管理（已完成）

`views/workers/index.vue` 已实现完整服务人员管理：

- **列表页**：姓名/工号 / 手机号 / 状态 / 评分 / **今日订单** / 技能 / 操作（详情/编辑/重置密码）（**无顶部统计卡、无投诉率列、无证书标签列**）
- **筛选**：关键词（姓名或手机号）/ 状态 / 技能（保洁/收废品/保洁和收废品）
- **新增/编辑弹窗**：v2.0 扩展字段 + 技能单选下拉 + 证书图片上传（健康证/技能证书**非必填**）；初始密码=手机号
- **详情抽屉**：基本信息 + 证书信息区（图片预览+有效期）+ 绩效统计（**无创收金额**）+ 投诉记录列表
- **重置密码**：确认弹窗 → `POST /workers/:id/reset-password`
- **API 封装**：`apps/admin/src/api/worker.ts` + `apps/admin/src/api/complaint.ts`

**P5.6 前端修复**：

| 问题 | 修复 |
|------|------|
| 手机号搜索无结果 | 纯数字关键词走 `phone` 参数而非 `name` |
| 新增员工 400 | `CreateWorkerDto` 补齐证书/联系人字段（forbidNonWhitelisted） |
| 健康证/技能证书误必填 | 移除前端必填校验 |

**P5.6 后端扩展**：

| 变更 | 说明 |
|------|------|
| `GET /workers?skillType=` | 技能筛选 |
...
### 18.10 P5.9 服务配置管理（已完成）

`views/config/services/index.vue` 已实现完整服务配置管理：

- **列表页**：序号 / 所属业务 / 服务名称 / 副标题 / 图标缩略图 / 排序 / 状态 / 创建时间 / 操作（编辑/删除）（**无价格相关列**）
- **筛选**：所属业务（保洁服务/废品回收/家政服务）/ 服务名称模糊搜索
- **新增/编辑弹窗**：所属业务 / 服务名称 / 副标题 / 图标 URL / 排序（**无计价方式/单价/参考价字段**）
- **启用/停用**：状态 Tag 点击 → 确认弹窗 → `PATCH /service-catalogs/:id/toggle`
- **删除**：确认弹窗 → `DELETE /service-catalogs/:id`
- **API 封装**：`apps/admin/src/api/service-catalog.ts`（fetch/create/update/delete/toggle）
