...
> **文档版本**：v1.5
> **编制日期**：2026-06-01
> **修订说明**：v1.5 新增 P2.1 需求确认：Auth 首阶段仅 Resident 微信登录 + JWT，采用纯 mock（`code -> 固定 openid`）并预留真实微信 `code2session` 接入点；Worker/Admin 登录在后续阶段实现。v1.4 经技术评审修正 3 处事实错误（SQLite 窗口函数 / wechat-sdk-node 依赖 / 依赖版本号），补充上线必需工程项（API 鉴权 / RBAC / COS 私有桶 / 状态变更日志 / 支付流水表 / 部署运维），重新论证 SCF 水印与 uni-app 选型；v1.3 §7 原型图 AI 代码生成源备注；v1.2 新增 §2.2 管理后台推荐骨架（vue3-element-admin / youlai，非强制约束）；v1.1 新增 §1.4 PocketBase 对照分析；v1.0 为初始技术栈选型结论
>
...
> **P2.1 认证范围冻结**：仅实现 Resident 微信登录 + JWT（access/refresh）；Worker/Admin 登录与鉴权不纳入 P2.1。
>  
> **P2.1 微信登录模式**：纯 mock（`code -> 固定 openid`），并在 `WechatModule` 保留真实微信 `code2session` 的扩展接口。
>
> **P2.2 用户 CRUD 口径冻结**：`Resident` 创建最小必填仅 `openid`；`Worker/Admin` 接口仅接收明文 `password`，并由服务端统一 `bcrypt` 写入 `passwordHash`，禁止客户端传递 `passwordHash`。

以下能力在一期实现，不做多余设计：

| 能力             | 一期实现方式                                                       | 备注                                                        |
| :--------------- | :----------------------------------------------------------------- | :---------------------------------------------------------- |
| 微信登录         | P2.1：Resident 侧 `wx.login` + 后端 mock（`code -> 固定 openid`）；后续切换真实 `code2session` | 首次下单弹窗补全姓名/手机号                                 |
...
| API 鉴权         | P2.1：Resident JWT（access + refresh）；Worker/Admin 鉴权后续接入   | **【v1.5 修订】** 与 P2.1 范围一致，先做单主体认证闭环       |
...
```
dayangyunjie-code/            # 编码仓库根（Git + npm workspaces）
├── apps/
...
├── docs/                     # CodingPlan、tech、Schema
├── requirement.md            # 需求文档（仓库根）
└── .workbuddy/               # WorkBuddy 工作记录（可选）

# 同级资料区（不在本仓库）: D:\coding\dayangyunjie\
```

> `packages/shared` 存放纯 TypeScript 类型定义（订单、用户、DTO 等），三个 App 均通过 workspace 引用，确保类型一致性。

> **补充确认（P1.4）**：居民端与员工端采用 **两个独立 uni-app 工程**（两个 workspace），分别配置 `tabBar` 与微信 `appid`；跨端共享仅通过 `packages/*`（如 `@dayangyunjie/shared`）复用，避免早期强行“单工程 + 条件编译”导致路由/入口复杂化。
...
| §3 居民端         | uni-app 页面（首页、预约流、订单列表/详情、评价、投诉）。**原型图为 AI 代码生成源**（资料区 `D:\coding\dayangyunjie\requirement\customer\` 下 19 张）    |
| §4 员工端         | uni-app 页面（任务列表/详情、服务指引弹窗、GPS 签到、拍照上传）。**原型图为 AI 代码生成源**（资料区 `D:\coding\dayangyunjie\requirement\uses\` 下 8 张） |
...
_本文档 v1.5，新增 P2.1 认证范围确认（仅 Resident 微信登录 + JWT，纯 mock openid，预留 code2session 扩展点）；v1.4 经技术评审修正 3 处事实错误（SQLite 窗口函数 / wechat-sdk-node / 依赖版本号）、补充上线必需工程项、重新论证 SCF 水印与语言选型论据，改进点以 `【v1.4 修订】`/`【v1.4 新增】` 标注；v1.3 §7 新增原型图 AI 代码生成源备注；v1.2 新增 §2.2 管理后台推荐骨架（非强制约束）；v1.1 新增 PocketBase 对照分析；v1.0 为初始选型结论。技术选型随项目演进而迭代。_
